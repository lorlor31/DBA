...
class WCCT_discount {
	...
	public function wcct_trigger_get_price( $get_price, $product_global ) {
		...
		/**  adding hook to remove multiple discount on single product on add to cart  */

		do_action( 'wcct_before_discount_price' );
		...
		do_action( 'wcct_after_discount_price' );
	...
	public function wcct_trigger_create_price( $sale_price, $product_global, $mode = 'basic', $regular_price = false, $parent_product = 0 ) {
		...
		if ( 1 == $deals_override['override'] ) {
			...
			$sale_price_date_from = get_post_meta( $product_global_id, '_sale_price_dates_from', true );
			$sale_price_date_to   = get_post_meta( $product_global_id, '_sale_price_dates_to', true );
			$today_date           = current_time( 'timestamp' );

			if ( '' != $sale_price_date_from && '' != $sale_price_date_to ) {
				if ( $today_date >= $sale_price_date_from && $today_date <= $sale_price_date_to ) {
					if ( $check_sale >= '0' ) {
