...
$price = apply_filters(
			...
			number_format( $product_display_price, wc_get_price_decimals(), wc_get_price_decimal_separator(), wc_get_price_thousand_separator() ),
			$product_display_price,
			wc_get_price_decimals(),
			wc_get_price_decimal_separator(),
			wc_get_price_thousand_separator()
		);
$product_display_price_html = ( $product_display_price < 0 ? '-' : '' )
					. sprintf( get_woocommerce_price_format(), '<span class="woocommerce-Price-currencySymbol">'
					. get_woocommerce_currency_symbol( get_woocommerce_currency() ) . '</span>', $price );
	...
	<table>
		<?php if ( $product_display_price > 0 ) : ?>
			<tr>
				...
				<td><div class="yith_wcp_group_option_total_base_price"><?php echo $product_display_price_html; ?></div></td>
			</tr>
		<?php endif; ?>
...
<div class="ywcp_customer_advice">
	<p><?php echo apply_filters( 'ywcp_customer_advice_text', __( 'The add to cart button is disabled because the following component is not selected:', 'yith-composite-products-for-woocommerce' ) ); ?><span class="ywcp_customer_advice_component_list"></span></p>
</div>
