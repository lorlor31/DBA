...
?>
    <div style="margin: 0 0 15px;">
        ...
        // "only if its current media query is matched"
        if ($matchMediaLoadStatus === 2) {
            if ( ! $linkHasDistinctiveMediaAttr ) {
                // The CSS file had a distinctive "media", but afterwards it was set to "all"
        ...
        if ($matchMediaLoadStatus && $matchMediaLoadCustomValue) { $data['row']['at_least_one_rule_set'] = true; }
        ?>
        ...
        <div data-style-handle="<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>"
             class="wpacu-handle-media-queries-load-field <?php if ($matchMediaLoadStatus === 1) { echo 'wpacu-is-visible'; } ?> wpacu-fade-in">
                <textarea id="<?php echo esc_attr($wpacuDataForTextAreaId); ?>"
                  ...
                  data-wpacu-is-empty-on-page-load="<?php echo (! $matchMediaLoadCustomValue) ? 'true' : 'false'; ?>"
                  <?php if (! $matchMediaLoadCustomValue) { echo 'disabled="disabled"'; } ?>
                  name="<?php echo WPACU_FORM_ASSETS_POST_KEY; ?>[styles][<?php echo htmlentities(esc_attr($data['row']['obj']->handle), ENT_QUOTES); ?>][media_query_load][value]"><?php echo esc_textarea($matchMediaLoadCustomValue); ?></textarea> &nbsp;<small style="vertical-align: top;">e.g. <em style="vertical-align: top;">screen and (max-width: 767px)</em></small>
