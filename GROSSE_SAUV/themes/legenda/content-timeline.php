...
<article <?php post_class($postClass); ?> id="post-<?php the_ID(); ?>" >	
	...
	<div class="post-info">
		<span class="posted-by"> <?php esc_html_e('Posted by', 'legenda');?> <?php the_author_posts_link(); ?></span> / 
	...
	<?php if($post_format == 'gallery'): ?>
        <?php if(count($gallery_filter['ids']) > 0): ?>
            <div class="nav-type-small<?php if (count($gallery_filter['ids'])>1): ?> images-slider<?php endif; ?> slider_id-<?php echo esc_attr($slider_id); ?>">
    ...
    <?php if($post_format != 'gallery'): ?>
        <div class="post-description">
        	<?php the_excerpt(); ?>
            <a href="<?php the_permalink(); ?>" class="more-link"><span class="button right read-more"><?php esc_html_e('Read More', 'legenda'); ?></span></a>
        </div>
    <?php elseif($post_format == 'gallery'): ?>
        <?php echo '<div class="post-description">' . $gallery_filter['filtered_content'] . '</div>'; ?>
    <?php endif; ?>
