...
if ( $_product instanceof WC_Product ) {
	...
	$option_price_html = '';
	if ( 'product' === $price_method ) {
		var_dump($_product->get_sale_price());
		$price_sale = '';
		$option_price = $_product_price;
		if ($option_price_html = ! $hide_product_prices) { 
		// ?'<small class="option-price"><del>' . wc_price( $option_price ) . '</del> ' . wc_price( $price_sale ) . '</small>' 
		// : '';
			if ($_product->get_sale_price()) {
			 echo '<small class="option-price"  style="text-decoration:line-through; font-size:16px color :#6F6F6F;font-weight: 700; margin : 7px 0 5px 0">'. wc_price(number_format($_product->get_regular_price(), 2, '.', '')) . '</small> </br> '.'<small class="option-price" style="color:#C92F00; margin : 7px 0 5px 0;"> '. $_product->get_sale_price() . '</small>' ;
			else {
			echo'<small class="option-price"  style="font-size:16px color :#6F6F6F;font-weight: 700; margin : 7px 0 5px 0">'. wc_price(number_format($_product->get_regular_price(), 2, '.', '')) . ' </br> '. '</small>';
			}
		else '';
		...
		// ? '<small class="option-price"  style="text-decoration:line-through; font-size:16px color :#6F6F6F;font-weight: 700; margin : 7px 0 5px 0">'. wc_price(number_format($_product->get_regular_price(), 2, '.', '')) . ' </br> '. '</small>'
		// .'<small class="option-price" style="color:#C92F00; margin : 7px 0 5px 0;"> '. $_product->get_sale_price() . '</small>' 
		// : '';
	} else {
