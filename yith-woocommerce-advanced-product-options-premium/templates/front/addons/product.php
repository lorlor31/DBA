...
if ( $_product instanceof WC_Product ) {
	...
	if ( 'product' === $price_method ) {
		...
		// Dans le cas d'une promotion via l'interface admin du produit
		// Affiche le prix sans promotion barré au dessus du prix avec promotion en rouge
		if (!$hide_product_prices) { 
			if ($_product->get_sale_price()) {
			$option_price_html =  '<small class="option-price"  
			 style="text-decoration:line-through; font-size:16px ;color :#6F6F6F;font-weight: 700; margin : 7px 0px 5px 0px ;">'
			. wc_price(number_format($_product->get_regular_price(), 2, '.', '')) 
			. '</small> <small class="option-price" style="color:#C92F00; "> '
			. wc_price(number_format($_product->get_sale_price(), 2, '.', '')) 
			. '</small>' ;
			} else {
			$option_price_html = '<small class="option-price"  
			style="font-size:16px; color :#6F6F6F;font-weight: 700; ">'
			. wc_price(number_format($_product->get_regular_price(), 2, '.', '')) . '</small>';
			}
		} else {
			echo '' ;
		}
