...
function find_parent_product_id($variation_id) {
...
if (is_admin()){
    ...
    */
	...
	// Fonction pour rediriger après le clonage de la commande
	function redirect_after_clone_order($order_id) {
		// Récupérez le lien de la nouvelle commande clonée
		$cloned_order_url = admin_url("post.php?post=$order_id&action=edit");

		// Redirigez l'utilisateur vers la nouvelle commande clonée
		wp_redirect($cloned_order_url);
		exit;
	}
	// Ajoutez l'action pour rediriger après le clonage
	add_action('vibe_clone_orders_after_clone', 'redirect_after_clone_order');
    ...
    */
	...
	// function display_postcode($order){
		...
		// if ($order->data["status"] == 'ywraq-new' || $order->data["status"] == 'ywraq-pending'){
			// echo '<p><strong>Code postal de livraison:</strong><br><span style="color:blue;">' . $dt_order["postcode"].'</span></p>';
		// }
	// }
	add_action('woocommerce_admin_order_data_after_shipping_address', 'display_postcode', 10, 1);
	function display_postcode($order){
		...
		// Afficher le code postal de livraison
		if ($order->data["status"] == 'ywraq-new' || $order->data["status"] == 'ywraq-pending'){
...
function sauvegarder_note_fournisseur( $post_id ) {
     ...
     */	
		...
		function custom_order_email_meta_box_callback( $post ) {
			...
			echo '<option value="relance1"' . (($relance1_sent === 'yes' || $relance2_sent === 'yes') ? ' disabled' : '') . '>Relance Mail 1</option>';
			echo '<option value="relance2"' . ($relance2_sent === 'yes' ? ' disabled' : '') . '>Relance Mail 2</option>';
			echo '<option value="relance3">Mail Libre</option>';
			echo '</select>';
			...
			<script type="text/javascript">
				...
				function fetchEmailData(orderId, template) {
					...
					$('#email_template').change(function() {
						...
						if (selectedTemplate === 'relance1' || selectedTemplate === 'relance2' || selectedTemplate === 'relance3') {
				...
				if ($order) {
					...
					switch ($template) {
						...
						case 'relance2':
							...
							break;
						case 'relance3':
							$data['emailContent'] = get_relance_mail_3_template();
							break;
					}
				} else {
			...
			function handle_send_custom_email_ajax() {
				...
				(($template === 'relance1' || $template === 'relance2') ? 'Devis n°' . $order_number . ' | Armoire PLUS' :
				($template === 'relance3' ? 'Dossier n°' . $order_number . ' | Armoire PLUS' : 'Dossier n°' . $order_number . ' | Armoire PLUS'));
				...
				if ($email_sent) {
					echo 'Email envoyé avec succès à ' . esc_html($billing_email);
					 $order = wc_get_order($order_id);
					if ($order && ($template === 'relance1' || $template === 'relance2')) {
						$note = 'Mail de relance ' . ($template === 'relance1' ? '1' : '2') . ' envoyé';
						$order->add_order_note($note,0,1); // true pour une note privée
						update_post_meta($order_id, '_mail_relance' . ($template === 'relance1' ? '1' : '2') . '_sent', 'yes');
					}
				} else {
...
function get_pdf_attachment_path($order_id) {
    ...
    // Ici, adaptez le format du nom de fichier selon la structure utilisée par YITH
    $pdf_file_name = "devissssss_" . $order_number . ".pdf";
    ...
    return '';
		...
		function get_relance_mail_1_template($order_id) {
			...
			$email_content =  'Bonjour,<br /><br />
					Suite à votre demande, nous vous avons envoyé votre devis le&nbsp;' . $order_date . ' numéro&nbsp;n° ' . $order_number . '.<br /><br />
					L\'avez-vous reçu, avez-vous eu le temps d\'en prendre connaissance et avez-vous des questions concernant celui-ci ?<br /><br />
					Afin de faciliter nos échanges, vous trouverez ci-dessous ma ligne directe.<br /><br />
					...
					<a href="mailto:contact@armoireplus.fr">contact@armoireplus.fr</a> - Site web <a href="http://www.armoireplus.fr/">www.armoireplus.fr</a> &nbsp;
					';
		...
		function get_relance_mail_2_template($order_id) {
			...
			$email_content =  'Bonjour,<br />
					<br />
					Suite &agrave; votre demande, nous avons eu le plaisir de vous adresser le&nbsp;' . $order_date . '&nbsp;&nbsp;notre devis n&deg; ' . $order_number . '&nbsp; que vous retrouverez en pi&egrave;ce jointe et pour lequel nous n&rsquo;avons pas &eacute;t&eacute; inform&eacute; de la suite &agrave; donner.<br />
					<br />
					Pouvez-vous nous dire si ce dossier peut-&ecirc;tre archiv&eacute; ou maintenu en attente ?<br />
					<br />
					Par ailleurs, par mesure de qualit&eacute;, si nous n&rsquo;&eacute;tions pas retenus, nous vous serions reconnaissants de bien vouloir nous en indiquer les raisons afin d&rsquo;am&eacute;liorer &agrave; l&rsquo;avenir notre offre commerciale.<br />
					<br />
					Cordialement, ' . $username . ' Armoire PLUS<br />
					<img src="https://www.armoireplus.fr/wp-content/uploads/2020/02/Logo-ArmoirePlus_small.png" style="height:40px; width:144px" /><br />
					T&eacute;l. : 05.31.61.98.32 / Fax : 05.17.47.54.02<br />
					<a href="mailto:contact@armoireplus.fr">contact@armoireplus.fr</a> - Site web <a href="http://www.armoireplus.fr/">www.armoireplus.fr</a> &nbsp;<br />
					<br />
					&nbsp;
					';
			 return $email_content;
...
add_action('save_post', 'enregistrer_mes_documents');
    ...
    */
    add_action( 'woocommerce_admin_order_data_after_order_details', 'display_customer_orders' );
    function display_customer_orders($order) {
        ...
        if ( $customer_id ) {
            ...
            if (!empty($orders)) {
                ...
                if ($total_orders > 1) {
                    echo '<table style="clear: both;"><thead><tr style="border-spacing:0"><th>Numéro de commande</th><th>Statut de la commande</th><th>Date de commande</th></tr></thead><tbody>';
                    $i = 1;
                    ...
                    echo '<h3 style="margin-top:250px;color:#D10000"><strong>C&#8217;est la première commande de ce client !<strong></h3>';
                }
            }
        }
    }
