							...
							<tr>
								...
								<td colspan="4">
									...
									<div id="addinfo_attribu">
										...
										//test laure
										$meta_data = $item->get_meta_data(); // Récupère toutes les métadonnées

										// Commence le tableau HTML
										echo '<table border="1" cellpadding="5" cellspacing="0">';
										echo '<thead>';
										echo '<tr>';
										echo '<th>Nom de la méta</th>';
										echo '<th>Valeur de la méta</th>';
										echo '</tr>';
										echo '</thead>';
										echo '<tbody>';
										
										// Boucle à travers les métadonnées pour les afficher dans le tableau
										foreach ( $meta_data as $meta ) {
											echo '<tr>';
											echo '<td>' . esc_html($meta->key) . '</td>'; // Escapement pour plus de sécurité
											echo '<td>' . esc_html($meta->value) . '</td>';
											echo '</tr>';
										}
										...
										if ($posPlusCar !== false) {
											...
											// error_log("objet item" .print_r($item,true));
											$excerpt = str_replace('<a href="#tab_description">Plus de Caractéristiques [...]</a>', '', $excerpt );
