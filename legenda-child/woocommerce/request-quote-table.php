...
if (isset($raq['yith_wapo_options'])) {
    foreach ($raq['yith_wapo_options'] as $addon_group) {
        foreach ($addon_group as $addon_key => $addon_value) {
            // Extraire l'ID de l'addon
            preg_match('/product-(\d+)/', $addon_value, $matches);
            $addon_product_id = absint($matches[1] ?? 0);

            if ($addon_product_id) {
                $addon_product = wc_get_product($addon_product_id);
                if ($addon_product) {
                    $addon_quantity = isset($raq['yith_wapo_product_qty'][$addon_key]) ? (int)$raq['yith_wapo_product_qty'][$addon_key] : 1;
                    $addon_total += floatval($addon_product->get_price()) * $addon_quantity;
                }
...
$final_price = $_product->get_price(); 
...
var_dump( "final price :",$final_price);
echo "<br><br>";
var_dump(  "regular_price :",$regular_price);echo "<br><br>";
var_dump( $is_discounted);
// Utiliser preg_match pour extraire la partie numérique
$price_string = $prince;
preg_match('/[\d,.]+/', $price_string, $matches);
$price_number = str_replace(',', '.', $matches[0]);
$price_float = floatval($price_number);
if ($is_discounted) {
} else {
    $price_float += $addon_total;
}

// var_dump('Total price including addons:', $raq);
