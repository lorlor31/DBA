...
if ( $_product instanceof WC_Product ) {
	...
	if ( 'product' === $price_method ) {
		...
		$option_price_html = ! $hide_product_prices ? '<small class="option-price"> test1' . wc_price( $option_price ) . '</small>' : '';
		...
		$option_price_html = ! $hide_product_prices ?
			'<small class="option-price"><del> test2' . wc_price( $option_price ) . '</del> ' . wc_price( $price_sale ) . '</small>' : '';
	} else {
