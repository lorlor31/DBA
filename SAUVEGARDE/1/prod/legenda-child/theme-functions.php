...
function et_send_msg_action() {
        ...
        if(isset($_GET['contact-submit'])) {
            ...
            if(trim($_GET['contact-ets']) === '') {
                $error_ets = true;
            } else{
                $ets = trim($_GET['contact-ets']);
            }
            ...
            if(!$error_name && !$error_email && !$error_msg && !$error_phone) {
               ...
               $body = "Vous avez été contacté par $name<br>$ets $phone<br><br>" . PHP_EOL . PHP_EOL;
...
function etheme_wc_get_product_labels( $product_id = '' ) {
	...
	$count_origin = 0;
		...
		if ($product->is_on_sale()) {
			if ($product->is_type('variable')) {
				$min_var_reg_price = floatval($product->get_variation_regular_price('min', true));
				$min_var_sale_price = floatval($product->get_variation_sale_price('min', true));
				if ($min_var_reg_price > 0) {
					$pourcentage = round((($min_var_reg_price - $min_var_sale_price) / $min_var_reg_price) * 100);
					echo '<div style="width:30px;position: absolute; top: 28px; right: 0; background-color: white; color: #c92e01; padding: 10px 5px; border:solid 1px #c92e01;border-radius: 50%;z-index:1;">-' . $pourcentage . '%</div>';
				}
			} else {
				...
				if ($prix_regulier > 0) {
					...
					echo '<div style="width:30px;position: absolute; top: ' . $count_origin . '; right: 0; background-color: white; color: #c92e01; padding: 10px 5px; border:solid 1px #c92e01;border-radius: 50%;z-index:1;">-' . $pourcentage . '%</div>';
				}
			}
		}
    if ( etheme_get_option('sale_icon') ):
		...
		$label_nf = get_post_meta( $product->get_id(), 'certificat_nf', true );
		if ( $label_nf == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_nf ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$label_nf_env = get_post_meta( $product->get_id(), 'certificat_nf_env', true );
		if ( $label_nf == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_nf_env ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$label_certifex = get_post_meta( $product->get_id(), 'certificat_topex', true );
		if ( $label_certifex == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_topex ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$label_silence = get_post_meta( $product->get_id(), 'label_silence', true );
		if ( $label_silence == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_silence ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$casier_exterieur_interieur = get_post_meta( $product->get_id(), 'casier_exterieur_interieur', true );
		if ( $casier_exterieur_interieur == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_casier_exterieur_interieur ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$casier_intemperies = get_post_meta( $product->get_id(), 'casier_intemperies', true );
		if ( $casier_intemperies == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_casier_intemperies ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		$casier_anti_vandalisme = get_post_meta( $product->get_id(), 'casier_anti_vandalisme', true );
		if ( $casier_anti_vandalisme == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_casier_anti_vandalisme ' . $position_top . '"></span>';
			$count_labels++;
		endif;
		// $casier_lavable = get_post_meta( $product->get_id(), 'casier_lavable', true );
		if ( $casier_lavable == 'enable' ):
			$position_top = 'position-' . $count_labels;
			$output .= '<span class="label-icon new-label-2 label_casier_lavable ' . $position_top . '"></span>';
			$count_labels++;
		endif;		
		if ( get_post_meta( $product->get_id(), 'valeur_assurable', true )):
            switch ( get_post_meta( $product->get_id(), 'valeur_assurable', true ) ) {
                case 'classe 1':
                    $class = 'label_val_assur_1';
					break;
                case 'classe 2':
                    $class = 'label_val_assur_2';
                    break;
				case 'classe 3':
					$class = 'label_val_assur_3';
					break;
                default:
                    $class = '';
                    break;
			...
			$position_top = 'position-' . $count_labels;
            $output .= '<span class="label-icon new-label-2 ' . $class . ' ' . $position_top . '"></span>';
			$count_labels++;
		endif;
    if ( etheme_get_option('new_icon') ):
        if ( etheme_product_is_new($product_id) ):
            switch ( get_post_meta( $product->get_id(), 'label_garantie', true ) ) {
                case '1':
					...
					break;
                case '2':
                    $class = 'new-label label-2-ans';
                    break;
				case '3':
					$class = 'new-label label-3-ans';
					break;
                case '5':
                    $class = 'new-label label-5-ans';
                    break;
                default:
                    $class = 'new-label';
                    break;
			...
			$position_top = 'position-' . $count_labels;
            $output .= '<span class="label-icon ' . $class . ' ' . $position_top . '">'.__( 'New!', 'legenda' ).'</span>';
			$count_labels++;
        endif;
		...
		// $label_eco = get_post_meta( $product->get_id(), 'label_eco', true );
		// if ( $label_eco == 'enable' ):
			// $position_top = 'position-' . $count_labels;
			// $output .= '<span class="label-icon new-label-2 label_eco ' . $position_top . '"></span>';
		// endif;
		$label_fast = get_post_meta( $product->get_id(), 'label_fast', true );
		if (($product->get_attribute('pa_delai-dexpedition') == 'Sous 0 semaine') || !empty($label_fast) ){
			$position_top = 'position-'. $count_labels;
