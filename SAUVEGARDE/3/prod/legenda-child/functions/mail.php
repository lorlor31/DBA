...
/**
 * Envoyer un email si commande annulée
 */
if ( class_exists('WC_Dropshipping_Orders') ) {
	...
	// AU CLIENT 
add_action('woocommerce_order_status_changed', 'custom_send_email_on_order_cancelled', 10, 4);
function custom_send_email_on_order_cancelled($order_id, $old_status, $new_status, $order) {
    if ($new_status === 'cancelled') {
		...
		$message = 'Bonjour,<br><br>
		Votre commande <b>' . $num_order . '</b> est annulée.<br><br>
		...
		wp_mail( $customer_email, $title, $html_message, $headers );		
    }
}
	// AU FOURNISSEUR
