...
<div style="background-color:#fff;<?php if ($is_email) { echo 'padding:15px;margin:15px 15%;'; } ?>">
            ...
            <td style="width:100%;vertical-align:top;border:1px solid #e7e7e7;padding:15px;">
                ...
                switch ( $current_shipping_method ) {
                    ...
                    case 'Formule ECO Plus':
                        ...
                        break;
                    case 'Messagerie':
                        $html .= '<b>Livraison en Semaine entre 8h et 16h30.</b>';
                        break;
                    default:
    ...
    // Affiche la note fournisseur dans le mail uniquement, pas dans le bon d'expédition
    if($is_email) {
        ...
        <tr style="text-align:left;">
            ...
            foreach ($addons_simple  as $addon) {
                // if (!isset($addon['product_id'])) {
					echo '<p style="font-size:7pt;' . ($is_email ? 'color:blue;' : '') . '">- ' . $addon['title'] . ' : <b>' . $addon['value'] . '</b></p>';
                // }
            }
            foreach ($addons_product  as $addon) {
                // if (isset($addon['product_id'])) {
				if ($is_email) {
					echo '<p style="font-size:7pt;color:red;">- ' . $addon['title'] . ' : <b>' . $addon['info'] . ' (' . $addon['product_addon_mpn'] . ', ' . $addon['product_addon_cog'] . ')</b></p>';
				} else {
					echo '<p style="font-size:7pt;">- ' . $addon['title'] . ' : <b>' . $addon['info'] . '</b></p>';
				}
                // }
            }
            ?>
			...
			<?php
				$product_mpn = get_product_mpn($product_data['product_id'], $wpdb);
				$product = wc_get_product($product_data['product_id']);
				$product_sku = $product->get_sku();
			?>
            <td style="font-size:8pt;border-bottom:1px solid #b8b8b8;padding:10px;">
				<?php
				echo $is_email ? (!empty($product_mpn) ? $product_mpn : $product_sku) : $product_sku;

				$addon_tval = 0;
				foreach ($addons_product  as $addon) {
					if (isset($addon['product_addon_id'])) {
						if ($is_email){
							echo '<p style="font-size:7pt;color:red;">+ ' . (!empty($addon['product_addon_mpn']) ? $addon['product_addon_mpn'] : $addon['product_addon_sku']) . '(x' . $addon['quantity'] . ')</b></p>';
						}else{
							echo '<p style="font-size:7pt;">+ ' . $addon['product_addon_sku'] . '(x' . $addon['quantity'] . ')</b></p>';
						}
					}
					$addon_val = $addon['product_addon_cog'] * $addon['quantity'];
