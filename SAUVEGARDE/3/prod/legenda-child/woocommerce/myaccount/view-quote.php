...
$show_total_column = $show_price && ywraq_show_element_on_list( 'total' );
...
?>
<div class="woocommerce-request-quote ywraq-view-quote-wrapper ywraq-status-<?php echo esc_attr( $order->get_status() ); ?>">
    <?php
        ...
        <p class="ywraq-buttons">
            ...
            if ( $print_button_pdf ) {
                ...
                <a class="ywraq-big-button ywraq-pdf-file"
                   ...
                   target="_blank"><?php esc_html_e( 'Télécharger le PDF', 'yith-woocommerce-request-a-quote' ); ?></a>
    ...
    <h2><?php echo $order->get_order_number(); ?></h2>
	<p>
    ...
    <?php do_action( 'woocommerce_quote_details_before_order_table', $order ); ?>

	<table class="shop_table order_details">
		<thead>
		<tr>
			<th class="product-name" colspan="<?php echo $colspan; ?>">Récapitulatif</th>
	...
	<div id="ywraq-reject-confirm" title="<?php esc_html_e( 'Reject Quote.', 'yith-woocommerce-request-a-quote' ); ?>"
		...
		<div class="ywraq-reject-confirm-wrapper">
			<p><?php printf( esc_html_x( 'You are about to reject the quote #%s.', 'The placeholder is the quote number', 'yith-woocommerce-request-a-quote' ), $order->get_order_number() ); ?>
				<br>
	...
	<div class="ywraq-additional-information">
			...
			if ( '' !== $customer_message || ! empty( $af4 ) || '' !== $admin_message ) :
				...
				if ( ! empty( $af4 ) ) :
					foreach ( $af4 as $key => $value ) :
						if ( ! empty( $value ) ):
							?>
							<!--<p class="ywraq-view-quote__customer-info hide_bastien">
        ...
        <div class="ywraq-customer-information">
            ...
            $shipping['shipping_company']   = $order->get_shipping_company();
            $shipping['shipping_address_1'] = $order->get_shipping_address_1();
            $shipping['shipping_address_2'] = $order->get_shipping_address_2();
            $shipping['shipping_name']      = $order->get_shipping_first_name();
            $shipping['shipping_surname']   = $order->get_shipping_last_name();
            $shipping['shipping_city']      = $order->get_shipping_city();
            $shipping['shipping_postcode']  = $order->get_shipping_postcode();
            $shipping['shipping_state']     = $order->get_shipping_state();
            $shipping['shipping_country']   = $order->get_shipping_country();
            $shipping['shipping_email']     = get_post_meta($order->get_id(), '_shipping_email', true);
            $shipping['shipping_email']     = empty( $shipping_email ) ? $billing['billing_email'] : $shipping_email;
            $shipping['shipping_phone']     = get_post_meta($order->get_id(), '_shipping_phone', true);
            $shipping['shipping_phone']     = empty( $shipping_phone ) ? $billing['billing_phone'] : $shipping_phone;
            $shipping['shipping_vat']       = $order->get_meta( 'ywraq_shipping_vat' );
            $shipping['shipping_vat']       = empty( $shipping_vat ) ? $order->get_meta( '_shipping_vat' ) : $shipping_vat;

            $content_printed = '';
            $content         = ( empty( $shipping['shipping_name'] ) && empty( $shipping['shipping_surname'] ) ) ? $user_name : $shipping['shipping_name'] . ' ' . $shipping['shipping_surname'];
            printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Name:', 'yith-woocommerce-request-a-quote' ), esc_html( $content ) );

            if ( $shipping['shipping_company'] ) {
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Company:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_company'] ) );
            }

            if ( $shipping['shipping_address_1'] || $shipping['shipping_address_2'] ) {

                $content = $shipping['shipping_address_1'] . ( $shipping['shipping_address_1'] ? '<br />' : '' ) . $shipping['shipping_address_2'];
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Address:', 'yith-woocommerce-request-a-quote' ), wp_kses_post( ( $content ) ) );
            }

            if ( $shipping['shipping_city'] ) {
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'City:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_city'] ) );
            }

            if ( $shipping['shipping_postcode'] ) {
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Postcode:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_postcode'] ) );
            }

            if ( $shipping['shipping_state'] ) {
                $states  = WC()->countries->get_states( $shipping['shipping_country'] );
                $state   = $states[ $shipping['shipping_state'] ];
                $content = ( '' === $state ) ? $shipping['shipping_state'] : $state;
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'State/Province:', 'yith-woocommerce-request-a-quote' ), esc_html( $content ) );
            }

            if ( $shipping['shipping_country'] ) {
                $countries = WC()->countries->get_countries();
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Country:', 'yith-woocommerce-request-a-quote' ), esc_html( $countries[ $shipping['shipping_country'] ] ) );
            }

            if ( $shipping['shipping_email'] ) {
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Email:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_email'] ) );
            }

            if ( $shipping['shipping_phone'] ) {

                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'Telephone:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_phone'] ) );
            }

            if ( $shipping['shipping_vat'] ) {
                printf( '<p class="ywraq-view-quote__customer-info"><strong>%s</strong> %s</p>', esc_html__( 'VAT:', 'yith-woocommerce-request-a-quote' ), esc_html( $shipping['shipping_vat'] ) );
            }
