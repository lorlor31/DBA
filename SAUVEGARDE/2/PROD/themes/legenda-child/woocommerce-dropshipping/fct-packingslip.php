...
function product_info_promo($prod_info, $supplier_info, $wpdb, $order_id, $is_email, $promo_ids, $promo_dates) {
    ...
    $nouvelle_promo = [66923,66927,66695,
66717,
66718,
684,
1304,
...
49229];
	...
	$attribut_promo = [ // sert a exclure les produits (hors accord avec le fournisseur)
	  /*  exemple mise en forme 
	  [
			'id_produit' => '683', id du produit, '*' pour pas d'id spécifique
			'name' => 'pa_coloris-de-corps', -- nom de l'attribut (slug) '*' pour pas de nom d'attribut spécifique
			'attr' => 'Beige 1015',   -- valeur de l'attribut (nom) '*' pour pas de valeur d'attribut spécifique
		], 
		*/
	  // [
			// 'id_produit' => '*', 
			// 'name' => 'pa_coloris-de-corps',
			// 'attr' => 'Beige 1015',
		// ]
	];
    $is_promo = false;
...
if (!function_exists('get_product_mpn')) {
    function get_product_mpn($product_id, $wpdb) {
		global $wpdb;
            ...
            if (is_array($meta_item)) {
                foreach ($meta_item as $meta_key => $meta_value) {
                    if (strpos($meta_value, 'product-') === 0) {
                        ...
                        $product_addon_id = $matches[1] ?? 0;

                        $product_addon_cog = get_post_meta($product_addon_id, '_wc_cog_cost', true) ?: "N/A";
                        $product_addon_mpn = get_product_mpn($product_addon_id, $wpdb);
						$product_addon = wc_get_product($product_addon_id);
						$product_addon_sku = $product_addon->get_sku();;
                        ...
                        $addons_info[] = [
                            ...
                            'product_addon_sku' => $product_addon_sku,
                            'product_addon_cog' => $product_addon_cog,
                            'product_addon_mpn' => $product_addon_mpn,
                        ];
                    }
                }
            }
...
if (!function_exists('get_attribute_name')) {
    function get_attribute_name($attribute_key) {
        ...
        $_taxonomy = str_replace('pa_', '', $attribute_key);
        $taxonomy = 'pa_' . $_taxonomy; // Just to ensure it is prefixed correctly
