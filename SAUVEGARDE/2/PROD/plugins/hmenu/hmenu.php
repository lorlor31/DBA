<?php
    ...
    */
    ...
    class hmenu_heroplugin
    {
        ...
        private $plugin_version = '1.16.2';
        private $plugin_prefix = 'hmenu_';
        private $first_release = '05-02-2015';
        private $last_update = '26-07-2022';
        ...
        public function __construct()
        {
            ...
            register_activation_hook(__FILE__, array(new hmenu_activate($this->plugin_name, $this->plugin_version, $this->plugin_dir), 'hmenu_setup_plugin')); //activate
            register_deactivation_hook(__FILE__, array(new hmenu_deactivate($this->plugin_name), 'hmenu_teardown_plugin')); //deactivate
            
            //detect if update required
            global $wpdb;
            if ($this->plugin_old_version == null && $hmenu_helper->hmenu_onAdmin()) { //only make the DB call if required
            ...
            add_filter('pre_set_site_transient_update_plugins', array(&$checkin, 'hmenu_check_in'));
            ...
            add_action('wp_ajax_hmenu_get_security_code', array(&$frame_sec, 'hmenu_get_security_code')); //admin: get frame security code
            add_action('wp_ajax_hmenu_process_file', array(&$file_processor, 'hmenu_process_file')); //admin: get frame security code
            ...
            add_action('wp_ajax_hmenu_autoGenerateViews', array(&$autogenerate, 'hmenu_create_views')); //admin: get plugin rating
        }
    }

    #INITIALISE THE PLUGIN CODE WHEN WP INITIALISES
    new hmenu_heroplugin();
