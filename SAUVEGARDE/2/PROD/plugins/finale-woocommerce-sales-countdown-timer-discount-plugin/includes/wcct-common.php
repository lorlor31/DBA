...
class WCCT_Common {
	...
	public static function load_rules_classes() {
		...
		include plugin_dir_path( WCCT_PLUGIN_FILE ) . '/rules/rules/wplms.php';
	 ...
	 *  @return	$post_id (int)
	 */
	public static function get_post_table_data( $trigger = 'all', $filters = array(), $count = '') {
		if ( $trigger == 'all' ) {
			$args = array(
	...
	public static function get_wcct_countdown() {
        ...
        /**
         * filter campaign by current wpml language on deal pages
         *
         */
	...
	public static function wcct_maybe_clear_cache() {
		 ...
		 */
		if ( function_exists( 'wp_cache_flush' ) && 1 === apply_filters( 'wcct_wp_cache_flush', 1 ) ) {
			wp_cache_flush();
		}
	...
	public static function default_rule_types( $types ) {
		...
		if ( defined('BP_COURSE_MOD_VERSION') ) {
		    $types[__( 'WPLMS', 'finale-woocommerce-sales-countdown-timer-discount-plugin' )] = array(
		        'wplms_course' => __( 'Courses', 'finale-woocommerce-sales-countdown-timer-discount-plugin' ),
		        'wplms_unit' => __( 'Units', 'finale-woocommerce-sales-countdown-timer-discount-plugin' ),
		        'wplms_quiz' => __( 'Quizzes', 'finale-woocommerce-sales-countdown-timer-discount-plugin' ),
		        'wplms_question' => __( 'Questions', 'finale-woocommerce-sales-countdown-timer-discount-plugin' ),
		    );
		}

		if(defined('WPLMS_ASSIGNMENTS_VERSION')){
		    $types[__( 'WPLMS', 'finale-woocommerce-sales-countdown-timer-discount-plugin' )]['wplms_assignment'] = __( 'Assignments', 'finale-woocommerce-sales-countdown-timer-discount-plugin' );
		}
	...
	public static function get_filter_args() {
		...
		if ( null !== filter_input( INPUT_GET, 's' ) ) {
			$args['s'] = filter_input( INPUT_GET, 's' );
		}
