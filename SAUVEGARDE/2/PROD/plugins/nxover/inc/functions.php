...
function nxover_option_delay($type, $opt, $method = '', $vinco = false, $order_id = null, $add_exp = true)
{
    ...
    foreach($opt as $opt_id => $v)
    {
        ...
        if ( $p_id && ($t = wc_get_product_terms($parent_id, 'pa_delai-dexpedition', ['fields' => 'names'])))
        {            
            ...
            foreach($m->value as $o)
            {
                foreach($o as $v)
                {
                    if ($p_id = nxover_addon_id($v))
                    {
						...
						error_log("Product : " .print_r($p, true));
                        ...
                        if ($p && $p->is_type('variation'))
                        {
                            ...
                            {
                                $attribute_display[] = wc_attribute_label($attribute_name) . ' : ' . $attribute_value;
                            }
                            $attributes_text = ', ' . implode(', ', $attribute_display);
                            $opt[$p_id] = [$p->get_name(), $p->get_sku(), $attributes_text];
                        }
                        else
                        {
...
//     print_r($opt);
    ...
    {                
        foreach($opt as $p)
        {
            ...
            {
                $row = str_replace($p[0], $p[0]. $p[2].' - '.$p[1], $row);
				$pos = strpos($row, '</strong>');
				if ($pos !== false)
				{
					$row = substr($row, $pos + strlen('</strong>'));
				}
                break;
            }
        }
