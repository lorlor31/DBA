...
class WC_Dropshipping_Orders {
	...
	public function make_pdf( $order_info, $supplier_info, $html, $file_name ) {
		...
		$pdf_path = $this->make_directory('dropshipping/' . $order_info['custom_order_number']);
		// generate a pdf for the current order and the current supplier
		$file = $pdf_path.'/BL'.$file_name;
		// create new PDF document
		require WC_CART_PDF_PATH . 'vendor/autoload.php';
        $mpdf = new \Mpdf\Mpdf( array( 'mode' => get_locale(), 'format' => 'A4', 'default_font' => 'dejavusans', 'margin_top' => '30px', 'margin_bottom' => '15px' ) );
        ...
        <table cellpadding="0" cellspacing="0" style="width:100%;margin:0;padding:0;">
            <tr>
                <td style="width:20%;text-align:left;">
                    <img src="'.$options['packing_slip_url_to_logo'].'" width="250" alt="Logo Armoire Plus">
                </td>
                <td style="width:80%;padding-top:10px;text-align:right;">
                    <h1 style="font-size:12pt;">FICHE EXPÉDITION</h1>
                    <p style="font-size:8pt;padding-top:10px;">
                        Réf. expédition : <strong>BL'.$order_info['custom_order_number'].'</strong><br>
        ...
        $mpdf->SetHTMLFooter('
        <div style="position:absolute;width:84%;left:8%;bottom:0;font-size:7pt;margin-bottom:30px;text-align:center;">
            <hr style="border-top:1px solid #ddd;height:1px;">
	...
	public function _send_order( $order_info, $supplier_info ) {
		...
		if ( $smtp_check == '1' || $std_mail == '1' || $std_mail == '' ) {
			...
			if ( $order->get_status() == 'completed' ) {
				$hdrs['Subject'] = 'Order #' . $order_info['number'] . ' est terminée ';
			} else if ( $order->get_status() == 'cancelled' ) {
				$hdrs['Subject'] = 'Order #' . $order_info['number'] . ' est annulée ';
			} else {
				$hdrs['Subject'] = 'Nouvelle commande #'.$order_info['number'].' de '.$from_name;
			}
			// Mail Subject
			/*
			...
			$headers  = 'From: ' . wp_specialchars_decode( $from_email ) . ' <' . $from_name . ">\r\n";
			if ( $cc_email == '0' ) {
			...
			if ( $order->get_status() == 'completed' || $order->get_status() == 'cancelled' ) {
				...
				error_log( 'Order Status ' . $order->get_status() );
                wp_mail( $hdrs['To'], $hdrs['Subject'], $message, $headers, $attachments['pdf_packingslip'] );
			}
		} else {
			$fullinfo = $order_info['options']['full_information'];
			// $bill = $order_info['options']['billing_phone'];
			...
			wp_mail( $hdrs['To'], $hdrs['Subject'], $email_message_html, $headers, $attachments['pdf_packingslip'] );
