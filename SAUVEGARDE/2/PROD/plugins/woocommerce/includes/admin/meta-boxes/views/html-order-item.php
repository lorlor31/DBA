...
<tr class="item <?php echo esc_attr( $row_class ); ?>" data-order_item_id="<?php echo esc_attr( $item_id ); ?>">
	...
	<td class="item_cost" width="1%" data-sort-value="<?php echo esc_attr( $order->get_item_subtotal( $item, false, true ) ); ?>">
		<div class="view">
			...
			$pv_public = $item['subtotal']/$item['quantity'];
			$pv_public_2 = $item['total']/$item['quantity'];
                // if (!empty ($product->get_sale_price()) && $product->get_sale_price()==$pv_public){
				// 	echo '<span style="color:red"><br>[PP ' . $product->get_regular_price() . '€]</span>';
				$regular_price = $product->get_regular_price();
				$regular_price_float = is_numeric($regular_price) ? floatval($regular_price) : 0.0;
				$formatted_regular_price = number_format($regular_price_float, 2);
				$pv_public_float = (float) $pv_public;
				$pv_public_float_2 = (float) $pv_public_2;
				$regular_price_float = (float) $product->get_regular_price();

				if ($pv_public_float < $regular_price_float){
                    echo '<span style="color:red"><br>[PP ' . $formatted_regular_price . '€]</span>';
                }elseif($pv_public_float_2 < $regular_price_float){
					echo '<span style="color:red"><br>[PROMO]['. $pv_public_float_2 . '€]</span>';
				}
				$product = $item->get_product();
				$product_id = $product->get_id();
				$supplier = $product->get_meta('supplier');
				$classe_expedition = get_post_meta( $post->ID, '_shipping_class', true );
				if ( $classe_expedition == 'bureau' && $supplier == "Vinco" ) {
					echo '<span style="color:red;">/!\ Si Drop</span> ' ;
				}
			?>
		</div>
	</td>
	<td class="quantity" width="1%">
		<div class="view">
			...
			if ( $refunded_qty ) {
				...
				$supplier = $product->get_meta('supplier');
				$classe_expedition = get_post_meta( $post->ID, '_shipping_class', true );
				if ($product_id == 7332 || $product_id == 7310 || $product_id == 7321 || $product_id == 7298){
					echo '<span style="color:red;">doit être vendu par un multiple de 3</span> ' ;
				}
			?>
		</div>
		<?php
