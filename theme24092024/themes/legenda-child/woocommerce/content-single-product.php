...
<?php if ( $product->is_type( array( 'simple', 'variable' ) ) && $product->get_sku() ) : ?>
    <span class="sku_wrapper">
        ...
        $parent_mpn = get_product_mpn($product->get_ID());

        if ($product->is_type('variable')) {
            $variations = $product->get_available_variations();
            $mpn_data = [];
            foreach ($variations as $variation) {
                $mpn_data[$variation['variation_id']] = get_product_mpn($variation['variation_id']);
            }
            $mpn_json = json_encode($mpn_data, JSON_HEX_QUOT);
        } else {
            $mpn_json = json_encode([$parent_mpn], JSON_HEX_QUOT);
        }
        ?>
        <div style="display:inline-block;<?= esc_attr($display_css) ?>">
            <i class="sku-title">Référence VINCO : </i>
            <span class="sku-mpn" data-mpn='<?= esc_attr($mpn_json) ?>' data-parent-mpn='<?= esc_attr($parent_mpn) ?>'><?= esc_html($parent_mpn) ?></span>
        </div>
    </span>
<?php endif; ?>
