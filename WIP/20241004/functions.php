...
function nxover_cart_options_shipping_delay($item, $method)
{
    ...
    $delay = false;
    $options = $item['yith_wapo_options'] ;
    ob_start();  
    var_dump($options);
    $output = ob_get_clean();
    $errorMessage = '   $options est :\n'. $output;
...
// //   array(1) {
// //     ["361-0"]=>
// //     string(14) "product-3648-1"
// //   }
// //   [1]=>
// //   array(1) {
// //     ["361-1"]=>
// //     string(14) "product-3650-1"
...
//         error_log($errorMessage);
    ...
    var_dump($opt);
    $output = ob_get_clean();
    $errorMessage = '   $opt est :\n'. $output;
    error_log($errorMessage);
            $delaiPourDebug = nxover_option_delay('S', $opt, $method->label, true) ;//8
            // error_log('$delaiPourDebug est ' . $delaiPourDebug);
            $delaiPourDebug2 = nxover_option_delay('M', $opt) ;//8
            // error_log('$delaiPourDebug2 est ' . $delaiPourDebug2);

            if ((nxover_option_delay('S', $opt, $method->label, true)) !== false)
            // error_log('$delaiPourDebugsssss est ' . $delaiPourDebug);

                $maxdelay = max($delaiPourDebug, (int) $delay);
        }
    }
    // error_log('$maxdelay est ' . $maxdelay);
    return $maxdelay;
}
