...
class Swatches
{
    ...
    public function frontend_display_swatch( $term_html, $link_attributes, $term_object, $filter ) {
        ...
        if ( in_array( $filter['e_name'], $taxonomies ) ) {
            ...
            $urlA = array();
            if (strpos($link_attributes, "?_gl") != "0") {
                $link_attributes = substr($link_attributes, 0, strpos($link_attributes, "?_gl")) . '"';
            }
            if (in_array($link_attributes, $urlA)) {
                ...
                $term_html = "<button onclick='window.location." . $link_attributes . "'>";
                $term_html .= '<span class="wpc-term-swatch-wrapper' . $wrapper_class . '">' . $swatch . '</span> </button>';
                $term_html .= '<span class="wpc-term-name">' . $term_object->name . '</span></button>';
            }
        }
