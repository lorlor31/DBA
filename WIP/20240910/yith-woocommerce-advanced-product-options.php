...
public function request_quote_view($item_data, $raq, $_product, $show_price = true) {
    error_log("ENTERED request_quote_view");
    ...
    if (isset($raq['yith_wapo_options'])) {
        ...
        if (defined('YITH_WAPO_VERSION') && version_compare(YITH_WAPO_VERSION, '3.7.0', '>=')) {
            ...
            foreach ($raq['yith_wapo_options'] as $index => $option) {
                error_log("FOR EACH index $index : " . print_r($option, true));
                ...
                foreach ($option as $key => $value) {
                    if ($key && '' !== $value) {
                        ...
                        $cart_item = $raq;
                        error_log("request_quote_view cart_item['yith_wapo_product_qty']: " . print_r($cart_item['yith_wapo_product_qty'], true));
                        error_log("request_quote_view cart_item['yith_wapo_qty_options']: " . print_r($cart_item['yith_wapo_qty_options'], true));
                        ...
                        error_log("request_quote_view key: $key");
                        error_log("request_quote_view value: $value");
                        error_log("request_quote_view addon_data_name: $addon_data_name");
                        error_log("request_quote_view addon_value BEFORE get_addon_display_on_cart: $addon_value");
                        
                        // Vérification avant get_addon_display_on_cart
                        $correct_addon_value = $addon_value;

                        $addon_value = $addon_cart_instance->get_addon_display_on_cart($addon_value, $addon_prices['sign'], $addon_prices['price'], $addon_prices['price'], $addon_prices['price_sale']);
                        error_log("request_quote_view addon_value AFTER get_addon_display_on_cart: $addon_value");
                        
                        // Log correct_addon_value for comparison
                        error_log("BEFORE get_addon_display_on_cart correct_addon_value: $correct_addon_value");
...
error_log("EXIT request_quote_view item_data: " . print_r($item_data, true));
			return $item_data;
		}
