...
if (is_admin()){
    ...
    function lw_woocommerce_gpf_feed_item_google( $feed_item, $product ) {
        ...
        $feed_item->sale_price_inc_tax = (float)$feed_item->sale_price_ex_tax * 1.2;
        $feed_item->regular_price_inc_tax = (float)$feed_item->regular_price_ex_tax * 1.2;
	...
	*/
	...
	function format_product_price( $price, $product ) {
		...
		if ( (is_woocommerce() || is_page('bon-plan')) && $product->is_on_sale() ) {
			if ($product->get_id() == 64032 || $product->get_id() == 63990) {
...
/**
 * Desactiver les commetnaires surt les images WP
 */
...
function add_details_to_addon_value_on_cart($value, $addon_id, $option_id, $key, $original_value, $cart_item) {
    ...
    // Vérifiez si le type de l'addon est 'product'
...
$option_product_qty = 1;
        if (isset($cart_item['yith_wapo_product_qty'][$addon_id . '-' . $option_id])) {
            ...
            $attribute_summary = isset($option_product->get_data()['attribute_summary']) ? $option_product->get_data()['attribute_summary'] : '';
 ...
 */
 ...
 /**
 * Module Force to login before checkout n'envoie pas de mail à l'inscription
 */
 // function custom_force_send_registration_email( $customer_id ) {
    // if ( $customer_id ) {
        // $mailer = WC()->mailer()->get_emails()['WC_Email_Customer_New_Account'];
        // if ( $mailer ) {
            // $mailer->trigger( $customer_id );
        // }
    // }
// }
// add_action( 'woocommerce_created_customer', 'custom_force_send_registration_email' );
