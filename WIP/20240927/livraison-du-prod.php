...
Class Livraison {
    ...
    function __construct() {
        ...
        self::$attribute = [
            [
                'name_pdt' => 'Bureau Droit - Ligne PLUS',
                'name_attr' => '*',
            ...
            [
                'name_pdt' => [
				'Bureau Compact Asymétrique - Ligne ÉLÉGANCE',
				'Bureau Compact Asymétrique - Ligne PLUS'
				],
                'name_attr' => '*',
                'attr' => ['bureau-180-x-80'],
                'plus' => 3
            ],
            [
                'name_pdt' => '*',
			...
			[
				'name_pdt' => [
					'Vestiaire Industrie Propre Gamme PREMIUM - case 30 cm', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 2 cases 60 cm', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 3 cases 90 cm', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 4 cases 120 cm', 
					'Vestiaire Industrie Salissante Gamme PREMIUM - 1 case L40', 
					'Vestiaire Industrie Salissante Gamme PREMIUM - 2 cases L80', 
					'Vestiaire Industrie Salissante Gamme PREMIUM - 3 cases L120', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 1 case 40 cm', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 2 cases 80 cm', 
					'Vestiaire Industrie Propre Gamme PREMIUM - 3 cases 120 cm'
				],
				'name_attr' => 'coloris-de-corps',
				'attr' => ['anthracite-7016'],
				'plus' => 4
			],
        ];
        self::$postcode = [
    ...
    foreach (self::$attribute as $attributeConfig) {
        $name_pdt = (is_array($attributeConfig['name_pdt']) ? $attributeConfig['name_pdt'] : [$attributeConfig['name_pdt']]);   
        if (in_array($productName, $name_pdt) || $attributeConfig['name_pdt'] == '*') {
			if ($attributeConfig['name_attr'] == '*') {
				foreach ($product->get_attributes() as $attrKey => $attrValue) {
					if (in_array($attrValue, $attributeConfig['attr'])) {
						$delai += $attributeConfig['plus'];
						break;
					}
				}
            } else {
                $attrSlug = 'pa_' . str_replace(' ', '-', strtolower($attributeConfig['name_attr']));
                $attrTermSlug = get_term_by('name', $product->get_attribute($attrSlug), $attrSlug)->slug ?? '';
                if (in_array($attrTermSlug, array_map('strtolower', $attributeConfig['attr']))) {
                    $delai += $attributeConfig['plus'];
                }
            }
        }
	...
	// Décaller Date si Jours fériée ou Weekend
		public function fourchette($date_livraison, $date_livraison_max, $max_delai, $supplier_name, $order){
			...
			if ($supplier_name == "vinco" || $supplier_name == "burocean" || $supplier_name == "ggi" || $supplier_name == "akaze" || $supplier_name == "trionyx" ) {
				$extraPeriodStart = '04/08/'.$currentYear;
				$extraPeriodEnd = '25/08/'.$currentYear;
				$extraPeriod = new DatePeriod(
					DateTime::createFromFormat('d/m/Y', $extraPeriodStart),
					new DateInterval('P1D'),
					DateTime::createFromFormat('d/m/Y', $extraPeriodEnd)
			...
			} elseif ($supplier_name == "armoire-plus"){
				$extraPeriodStart = '09/08/'.$currentYear;
				$extraPeriodEnd = '18/08/'.$currentYear;
				$extraPeriod = new DatePeriod(
					DateTime::createFromFormat('d/m/Y', $extraPeriodStart),
					new DateInterval('P1D'),
					DateTime::createFromFormat('d/m/Y', $extraPeriodEnd)
    ...
    function email_shipping_delay_partial( $order, $sent_to_admin, $plain_text, $email ) {
        if ( $email->id != 'customer_note' ) {
            ...
            if ( ($status == 'partial-shipped' || $status == 'completed') ) {
				...
				// Récupérer toutes les métadonnées du shipping_method
				...
				foreach ($all_meta as $meta) {
                    ...
                    foreach ( $order_items as $item ) {
                        ...
                        if ( strpos( $shipping_item, $item->get_name() ) !== false && in_array(get_post_meta($item_id, '_sku', true), $current_shipped_sku) ) {
                            ...
                            if ( $status == 'completed' ) {
                                $delai = intval(preg_replace('/[^0-9.]/', '', wc_get_product_terms( $product_id, 'pa_delai-dexpedition', array( 'fields' => 'names' ) )[0] ) );
        ...
        if ( $status == 'processing' || $status == 'partial-shipped' || $status == 'completed' ) {
            ...
            foreach ( $shipping_methods as $shipping_method ) {
                ...
                switch ( $method_name ) {
                    ...
                    default:
                        $html = '<b style="color:#7f1321;"><span style="color:#333333;">Livraison entre '. $date_livraison . ' et ' . $date_livraison_max .'</b><br>';
                }
				if (!empty($custom_shipping_date)) {
    ...
    function cart_shipping_delay_order( $label, $method ) {
        ...
        foreach ( $cart as $cart_item ) {
            ...
            if (strpos($product_list, $product->get_name()) !== false) {
				...
				$delai = Livraison::getInstance()->getLivraison($product_id, $is_variation, $cart_item['variation'], $delai, $method->label);
                $max_delai = $delai > $max_delai ? $delai : $max_delai;
                // NXOVER [[
        ...
        foreach ( $shipping_methods as $shipping_method ) {
            ...
            foreach ( $order_items as $item ) {
                if ( strpos( $shipping_item, $item->get_name() ) !== false ) {
                    ...
                    $image_tag = '<img src="' . $image[0] . '" width="60" height="60">';
                    $supplier_name = get_post_meta($item['product_id'], 'supplier', true);
// FCT Dupliquée quote / proforma header					
$addons_html = '';
$addons = [];
foreach ($item->get_meta_data() as $meta) {
    $meta_data = $meta->get_data();  // Récupère les données sous forme de tableau
    if ($meta_data['key'] === '_ywraq_wc_ywapo') {
        // Si on rencontre '_ywraq_wc_ywapo', récupérez les métadonnées précédentes selon le nombre d'entrées
        $number_of_addons = count($meta_data['value']);  // Compte le nombre d'entrées dans le tableau '_ywraq_wc_ywapo'
        $temp_html = [];
        for ($i = 1; $i <= $number_of_addons; $i++) {
            if (isset($addons[count($addons) - $i])) {  // Récupère les métadonnées correctes du tableau
                $addon_info = $addons[count($addons) - $i];
                $key = $addon_info['key'];
                $value = $addon_info['value'];
                // Formatage des valeurs pour retirer les parenthèses autour des prix
                $value = preg_replace('/\s*\([^)]*\)/', '', $value);
                array_unshift($temp_html, "$key: $value");  // Ajoute au début du tableau temporaire pour maintenir l'ordre
            }
        }
        $addons_html .= ($addons_html ? ', ' : '') . implode(', ', $temp_html);  // Convertit le tableau en chaîne séparée par des virgules
        $addons = [];  // Réinitialisez le tableau d'addons après les avoir traités
    } else {
        $addons[] = ['key' => $meta_data['key'], 'value' => $meta_data['value']]; // Ajoute au tableau des addons
    }
}
$references .=  '<p class="line-product">' . $image_tag . '<span><b>' . $product->get_name('edit') . '</b>';
if (sizeof($attributes) > 0) {
    $attributes_html = '<br><small style="color:#797979;">' . wc_get_formatted_variation($attributes, true) . '</small>';
    $references .= $attributes_html;
}
if (!empty($addons_html)) {
    $addons_html_formatted = '<small style="color:#797979;">' . ($addons_html ? (sizeof($attributes) > 0 ? ", " : " ") . $addons_html : "") . '</small>';
    $references .= $addons_html_formatted;
}
$references .= '</span></p>';
                    // $references .= sizeof($attributes) > 0 ?
                        // '<p class="line-product">' . $image_tag . '<span><b>' . $product->get_name('edit') . '</b><br><small style="color:#797979;">' . wc_get_formatted_variation($attributes, true) . '</small></span></p>' :
                        // '<p class="line-product">' . $image_tag . '<b>' . $product->get_name('edit') . '</b></p>';
                    $delai = intval(preg_replace('/[^0-9.]/', '', ''.wc_get_product_terms( $product_id, 'pa_delai-dexpedition', array( 'fields' => 'names' ) )[0] ) );
