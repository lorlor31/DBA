...
if ( ! class_exists( 'YITH_YWRAQ_Send_Quote_Reminder' ) ) {
	...
	class YITH_YWRAQ_Send_Quote_Reminder extends WC_Email {
		...
		/**
		 * Check if the emails should be sent as BCC
		 *
		 * @var bool
		 */
		...
		public function init_form_fields() {
			$this->form_fields = array(
				...
				'days_before_expire' => array(
					'title'             => __( 'Send this email: ', 'yith-woocommerce-request-a-quote' ),
					'type'              => 'text',
					'description'       => __( 'Choose when to send this reminder.', 'yith-woocommerce-request-a-quote' ),
					'placeholder'       => '',
					'css'               => 'width:50px;',
					'default'           => 2,
					'custom_attributes' => array(
						'data-description' => _x( 'days before the quote expiration', 'additional info inside the email option', 'yith-woocommerce-request-a-quote' ),
					),
				),
