...
class YITH_YWRAQ_Order_Request {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_before_order_object_save', array( $this, 'send_quote_wpml_fix' ) );
	...
	/**
	 * Fix send quote email with wpml
	 *
	 * @return void
	 */
	public function send_quote_wpml_fix() {

		global $sitepress;
		if($sitepress){
			if( isset($_REQUEST['yit_metaboxes']['_ywraq_safe_submit_field']) && 'send_quote' === $_REQUEST['yit_metaboxes']['_ywraq_safe_submit_field'] && ! $this->quote_sent){
				do_action( 'send_quote_mail', $_REQUEST['post_ID'] );
	...
	public function set_quote_ready_for_pay_now( $response, $order, $status = '' ) {
		...
		if ( yith_plugin_fw_is_true( $pay_now ) && isset( $_GET['pay_for_order'] ) ) {
			if ( is_array( $status ) ) {
				if ( array_intersect( $status, array( 'ywraq-pending', 'pending', 'ywraq-accepted' ) ) ) {
					$response = true;
				}
			} else {
				if ( in_array( $status,  array( 'ywraq-pending','pending','ywraq-accepted'),true ) ) {
					$response = true;
				}
			}
		}
	...
	public function empty_cart( $order_id ) {
		if ( $this->is_quote( $order_id ) && ! is_admin() ) {
			...
			/**
			 * APPLY_FILTERS: ywraq_change_status_after_cancel_order
			 *
			 ...
			 *
			 */
			$new_status = apply_filters( 'ywraq_change_status_after_cancel_order', 'cancelled', $order );
	...
	public function change_order_status() {
		...
		if ( isset( $request['login'] ) && 'yes' === $request['login'] && ! is_user_logged_in() ) {
			unset( $request['login'] );

			if ( 'accepted' === $request['status'] ) {
				$pay_now  = ywraq_get_payment_option( 'ywraq_pay_quote_now', $order );
				$page_url = ( ywraq_is_true( $pay_now ) ) ? $order->get_checkout_payment_url( false ) : YITH_Request_Quote()->get_raq_page_url();
			} else {
				$page_url = YITH_Request_Quote()->get_raq_page_url();
			}
			$page_url = add_query_arg( $request, $page_url );

			$redirect = add_query_arg( 'redirect_to', rawurlencode( $page_url ), wc_get_page_permalink( 'myaccount' ) );
		...
		if (apply_filters('ywraq_override_shipping_method', ywraq_is_true($override_shipping))) {
			...
			if ($shipping_items) {
				...
				foreach ($rates as $key => $rate) {
					$rate_articles_meta = $rate->get_meta_data();
					$rate_articles = isset($rate_articles_meta['Articles']) ? $rate_articles_meta['Articles'] : '';
					$decoded_rate_articles = html_entity_decode($rate_articles, ENT_QUOTES | ENT_HTML5, 'UTF-8');
					$decoded_rate_articles_without_quantity = preg_replace('/,.*$/', '', $decoded_rate_articles);
					$rate_articles_without_quantity = mb_ereg_replace('\s*[x×]\s*\d+\s*$', '', $decoded_rate_articles_without_quantity);
					if (in_array($rate_articles_without_quantity, $matched_shipping_items)) {
						continue; // Passer au shipping_item suivant si celui-ci a déjà été apparié
					}
					// Fin modif Nico pour update 

					foreach ($shipping_items as $shipping_item) {
						// Modif Nico pour update 
						if ($shipping_item !== null && method_exists($shipping_item, 'get_method_id')) {
							// Fin modif Nico pour update 
							...
							// Modif Nico pour update 
							$shipping_item_articles_meta = $shipping_item->get_meta('Articles');
							$decoded_shipping_item_articles = html_entity_decode($shipping_item_articles_meta, ENT_QUOTES | ENT_HTML5, 'UTF-8');
							$shipping_item_articles_without_quantity = preg_replace('/,.*$/', '', $decoded_shipping_item_articles);
							$shipping_item_articles_final = mb_ereg_replace('\s*[x×]\s*\d+\s*$', '', $shipping_item_articles_without_quantity);
							$original_string = $decoded_shipping_item_articles;
							$parts = explode(',', $original_string);
							$famille_pdt_supplier = array();
							foreach ($parts as $part) {
								$cleaned_part = mb_ereg_replace('\s*[x×]\s*\d+\s*$', '', trim($part));
								$famille_pdt_supplier[] = $cleaned_part;
							}
							if (in_array($shipping_item_articles_final, $matched_shipping_items)) {
								continue; // Passer au shipping_item suivant si celui-ci a déjà été apparié
							}
							// Vérification de la correspondance entre les métadonnées 'Articles'
							if ($shipping_item_articles_final === $rate_articles_without_quantity) {
								// Mise à jour de $new_rates avec les informations de $shipping_item
								$new_rates[$key] = $rates[$key];
								$new_rates[$key]->cost = floatval($shipping_item->get_total());
								$new_rates[$key]->label = $shipping_item->get_name();
								// Fin modif Nico pour update 



								$tt = $shipping_item->get_taxes();
								// UPDATE MODIF PAR RAP A LA PRECEDENTE
								if (! empty($tt['total'])) {
								...
								$matched_shipping_items = array_merge($matched_shipping_items, $famille_pdt_supplier);
								break; // Arrêt de la boucle après la correspondance

							}
						}
					}
					// MODIF DE L UPDATE LAISSEE, ANC VERSION NON REPORTEE - A VERIFIER
					foreach ($rates as $key => $rate) {
