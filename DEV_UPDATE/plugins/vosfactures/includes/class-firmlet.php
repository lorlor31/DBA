...
class Vosfactures {
	...
	private function define_admin_hooks( $plugin_admin ) {
		...
		$this->loader->add_action( 'boxtal_connect_order_shipped', $plugin_admin, 'firmlet_boxtal_connect_order_shipped', 10, 1 );

		$options = get_option( 'woocommerce_firmlet_settings' );

		if ( isset($options['show_tax_no_input']) && $options['show_tax_no_input'] == 'yes' ) {
		...
		$this->loader->add_filter( 'woocommerce_shop_order_list_table_columns', $plugin_admin, 'vf_shop_order_column' );
		$this->loader->add_action( 'woocommerce_shop_order_list_table_custom_column', $plugin_admin, 'vf_orders_list_column_content', 10, 2 );
