...
class VosfacturesAdmin {
	...
	public function enqueue_styles() {
		 ...
		 * class.
		 */
		wp_register_style( 'custom_wp_admin_css', plugin_dir_url( __FILE__ ) . 'css/firmlet-admin.css', false, '1.3.9' );
	...
	public function firmlet_boxtal_connect_order_shipped( $order_id ) {
		if ( get_option( 'woocommerce_firmlet_settings' )['auto_issue'] != 'order_processing' ) {
			return;
		}

		$logger = wc_get_logger();
		$logger->debug( 'boxtal_connect_order_shipped [' . $order_id . '] was triggered', array( 'source' => 'vosfactures' ) );
		$this->firmlet_after_new_order( $order_id );
	}
