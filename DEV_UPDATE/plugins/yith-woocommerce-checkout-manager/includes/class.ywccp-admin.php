	...
	class YWCCP_Admin {
		...
		public function fields_general_section( $options ) {
			...
			if ( isset( $_GET['page'] ) && isset( $_GET['tab'] ) && sanitize_text_field( wp_unslash( $_GET['page'] ) ) === $this->panel_page && 'fields' === sanitize_text_field( wp_unslash( $_GET['tab'] ) )
				...
				$sections      = array(
					'billing'    => __( 'Billing fields', 'yith-woocommerce-checkout-manager' ),
					'shipping'   => __( 'Shipping fields', 'yith-woocommerce-checkout-manager' ),
					'additional' => __( 'Additional fields', 'yith-woocommerce-checkout-manager' ),
				);
