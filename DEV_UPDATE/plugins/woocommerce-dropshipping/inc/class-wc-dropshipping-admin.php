...
class WC_Dropshipping_Admin {
	...
	public function column_content( $blank, $column_name, $term_id ) {
		...
		switch ( $column_name ) {
			...
			*/
			...
			case 'inventory':
				echo '<p><a title="Import ' . $ds['name'] . '&apos;s Inventory Status in .CSV Format" href="' . admin_url( 'admin-ajax.php' ) . '?action=get_CSV_upload_form&width=600&height=350&term_id=' . $term_id . '" class="thickbox button-primary csvwindow" term_id="' . $term_id . '" >' . __( ' Import Inventory .CSV', 'woocommerce-dropshipping' ) . '</a></p>';
	...
	public function display_add_form_fields( $data ) {
        ...
        @media screen and (min-width: 1300px) {
			...
			<div class="form-field term-account_number-wrap">
				<label for="account_number" >' . __( ' Account #', 'woocommerce-dropshipping' ) . '</label>
				<input type="text" size="40" name="account_number" value="' . $data['account_number'] . '" />
				<p>' . __( 'Your store&apos;s account number with this supplier. Leave blank if you don&apos;t have an account number', 'woocommerce-dropshipping' ) . '</p>
			</div>
			<div class="form-field form-required term-order_email_addresses-wrap">
				<label for="order_email_addresses" >' . __( ' Email Addresses', 'woocommerce-dropshipping' ) . '</label>
				<input type="text" size="40" name="order_email_addresses" value="' . $data['order_email_addresses'] . '"  />
				<p>' . __( 'When a customer purchases a product from you, the supplier will be sent an notification via email. List the supplier&apos;s email addresses that should be notified when a new order is placed.', 'woocommerce-dropshipping' ) . '</p>
			</div>';
			...
			echo '
				<div class="form-field form-required term_specific_delivery_location_wrap">
					...
					margin-bottom: 7px;" > ' . __( 'Enable specific delivery location for the supplier.', 'woocommerce-dropshipping' ) . '</label>';
				...
				<div class="inner-toggle specific_delivery_location" style="display:none;">
					...
					<input type="text" class="td_address" size="40"  name="address_line1" value="' . __( $data['address_line1'], 'woocommerce-dropshipping' ) . '" /></br></br>

					<label for="address_line2" >Address line 2</label>
					<input type="text" class="td_address" size="40"  name="address_line2" value="' . __( $data['address_line2'], 'woocommerce-dropshipping' ) . '"  /></br></br>

					<label for="supplier_city" >City</label>
					<input type="text" class="td_address" size="40"  name="supplier_city" value="' . __( $data['supplier_city'], 'woocommerce-dropshipping' ) . '"  /></br></br>

					<label for="country_state" >Country/State</label>
					<input type="text" class="td_address" size="40"  name="country_state" value="' . __( $data['country_state'], 'woocommerce-dropshipping' ) . '" /></br></br>

					<label for="postcode_zip" >Postcode/ZIP</label>
					<input type="text" class="td_address" size="40"  name="postcode_zip" value="' . __( $data['postcode_zip'], 'woocommerce-dropshipping' ) . '"/></br></br>
	...
	public function display_edit_form_fields( $data ) {
		...
		.validate{
			margin-top: -190px;
					...
					<tr class="term-account_number-wrap">
						<th><label for="account_number" >' . __( ' Account #', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="40" name="account_number" value="' . $data['account_number'] . '" />
						<p>' . __( ' Your store&apos;s account number with this supplier. Leave blank if you don&apos;t have an account number', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>

					<tr  class="term-order_email_addresses-wrap">
						<th><label for="order_email_addresses" >' . __( ' Email Addresses', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="40" name="order_email_addresses" value="' . $data['order_email_addresses'] . '"  />
						<p>' . __( ' When a customer purchases a product from you, the supplier will be sent an notification via email. List the supplier&apos;s email addresses that should be notified when a new order is placed.', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>';
					...
					echo '<tr class="term_specific_delivery_location_wrap">
						...
						<th>
							<label for="specific_delivery_location" > ' . __( 'Enable specific delivery location for the supplier.', 'woocommerce-dropshipping' ) . '</label>
						</th>
					...
					<div class="hide_show_div_spe_del" style="display:none; background-color:red;">
						<tr class="inner-toggle specific_delivery_location" ' . $speci_del_style_disply . '>
							...
							<th><label for="address_line1" >' . __( 'Address line 1', 'woocommerce-dropshipping' ) . '</label></th>
							<td><input type="text" class="td_address" size="40" name="address_line1" value="' . __( $data['address_line1'], 'woocommerce-dropshipping' ) . '"  /></td></br></br>
						</tr>

						<tr class="inner-toggle specific_delivery_location" ' . $speci_del_style_disply . '>
							<th><label for="address_line2" >' . __( ' Address line 2', 'woocommerce-dropshipping' ) . '</label></th>
							<td><input type="text" class="td_address" size="40" name="address_line2" value="' . __( $data['address_line2'], 'woocommerce-dropshipping' ) . '"  /></td></br></br>
						</tr>

						<tr class="inner-toggle specific_delivery_location" ' . $speci_del_style_disply . '>
							<th><label for="supplier_city" >' . __( 'City', 'woocommerce-dropshipping' ) . '</label></th>
							<td><input type="text" class="td_address" size="40" name="supplier_city" value="' . __( $data['supplier_city'], 'woocommerce-dropshipping' ) . '"  /></td></br></br>
						</tr>

						<tr class="inner-toggle specific_delivery_location" ' . $speci_del_style_disply . '>
							<th><label for="country_state" >' . __( 'Country/State', 'woocommerce-dropshipping' ) . '</label></th>
							<td><input type="text" class="td_address" size="40" name="country_state" value="' . __( $data['country_state'], 'woocommerce-dropshipping' ) . '"   /></td></br></br>
						</tr>

						<tr class="inner-toggle specific_delivery_location" ' . $speci_del_style_disply . '>
							<th><label for="postcode_zip" >' . __( 'Postcode/ZIP', 'woocommerce-dropshipping' ) . '</label></th>
							<td><input type="text" class="td_address" size="40" name="postcode_zip" value="' . __( $data['postcode_zip'], 'woocommerce-dropshipping' ). '"  /></td></br></br>

						</tr>';

				echo '</table>

				<h3>' . __( 'Supplier Inventory CSV Import Settings', 'woocommerce-dropshipping' ) . '</h3>
				<p>' . __( '(If you do not receive inventory statuses from your supplier in the form of a .CSV file, leave these settings blank)', 'woocommerce-dropshipping' ) . '</p>
				<table class="form-table">
					<tr  class="term-csv_delimiter-wrap">
						<th><label for="csv_delimiter" >' . __( 'CSV File Column Delimiter', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="2" name="csv_delimiter" value="' . $data['csv_delimiter'] . '" />
						<p>' . __( 'Please indicate what character is used to separate fields in the CSV file. Normally this is a comma', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>



					<tr  class=" term-column_sku-wrap">
						<th><label for="csv_column_sku" >' . __( 'CSV SKU Column #', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="2" name="csv_column_sku" value="' . $data['csv_column_sku'] . '" />
						<p>' . __( 'Please indicate which column in the CSV file corresponds to product SKUs. Note that this should be the same SKU that the manufacturer uses. WooCommerce Dropshipping will automatically add the SKU code for products from this suppler when you upload a .CSV file', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>



					<tr  class=" term-csv_type-wrap">
						<th><label for="csv_type">' . __( 'CSV Type', 'woocommerce-dropshipping' ) . '</label></th>
		...
		echo '</select>
						<p>' . __( 'Please indicate how the .CSV file&apos;s data should be read. ', 'woocommerce-dropshipping' ) . '<br /><br /><b>' . __( 'Quantity on Hand ', 'woocommerce-dropshipping' ) . '</b>' . __( '- If your supplier sends you a .CSV file that contains the quantity that they have remaining in their inventory, you should use this method. Any number above zero indicates that the product is still in stock.', 'woocommerce-dropshipping' ) . '<br /><b>' . __( ' In-Stock Indicator ', 'woocommerce-dropshipping' ) . '</b> ' . __( '- Use this method if your supplier sends you a .CSV file that includes a column indicating whether or not a product is in stock.  This is typically in either a Y/N or 1/0 format to indicate whether or not the product is in stock.', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>
					<tr  class="csv_quantity csv_types">
						<th><label for="csv_column_qty" >' . __( 'CSV Inventory Quantity Column #', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="2" name="csv_column_qty" value="' . $data['csv_column_qty'] . '" />
						<p>' . __( 'Please indicate which column in the .CSV file corresponds to the quantity of inventory available', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>


					<tr  class="csv_indicator csv_types">
						<th><label for="csv_column_indicator" >' . __( 'In-Stock Indicator Column #', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="2" name="csv_column_indicator" value="' . $data['csv_column_indicator'] . '" />
						<p>' . __( 'Please indicate which column in the .CSV file indicates whether or not a product is in stock', 'woocommerce-dropshipping' ) . '</p></td>
					</tr>

					<tr  class="csv_indicator csv_types">
						<th><label for="csv_indicator_instock" >' . __( 'In-Stock Indicator Value', 'woocommerce-dropshipping' ) . '</label></th>
						<td><input type="text" size="2" name="csv_indicator_instock" value="' . $data['csv_indicator_instock'] . '" />
						<p>' . __( 'Please input the value (ie. Y or 1) in the column defined above that indicates whether or not a product is in stock', 'woocommerce-dropshipping' ) . ' </p></td>
					</tr>';
	}
				...
				if ( ! empty( $username ) && ! empty( $email ) && ! $user_id && email_exists( $email ) == false ) {
					...
					if ( isset( $email_supplier ) ) {
						if ( $email_supplier == '1' ) {
							...
							$message = '<html><body>';
							$message .= '<h1 style="color:#f40;">' . __( 'Hi ' . $user_id_role->display_name . ', woocommerce-dropshipping' ) . '</h1>';

							$message .= '<p style="color:#080;font-size:15px;">' . __( 'Thanks For Registration', 'woocommerce-dropshipping' ) . '</p>';

							// $message .= '<p style="disply:none">Your Email:&nbsp;'. $email .'</p>';
							$message .= '<p>' . __( 'Your User Name:&nbsp;' . $user_id_role->display_name . ', woocommerce-dropshipping' ) . '</p>';
							$message .= '<p>' . __( 'Your Password:&nbsp;' . $random_password . ', woocommerce-dropshipping' ) .'</p>';
							$message .= '<p>' . __( 'Change Your Password Once you login', 'woocommerce-dropshipping' ) .' </p>';
							$message .= '<p>' . __( 'Login URL: ' . $loginurl . ', woocommerce-dropshipping' ) . '</p>';
							$message .= '</body></html>';
