 ...
 */
class ExpandedStructuredData {

	// Dependencies.
	protected ProductFeedItemFactory $feed_item_factory;

	/**
	 * Additional markup to be output for the currently processing product.
	 *
	 * @var array
	 */
	protected array $additional_product_markup = [];

	/**
	 * The cache TTL for calculated data.
	 *
	 * @var int
	 */
	 ...
	 */
	public function __construct( ProductFeedItemFactory $feed_item_factory ) {
	...
	public function initialise(): void {
		...
		add_action( 'woocommerce_single_product_summary', [ $this, 'generate_schema_products' ] );
	 ...
	 */
	public function generate_schema_products(): void {
	 ...
	 * @param WC_Product $wc_product
	 */
	private function generate_simple_schema_product( $wc_product ): void {
	 ...
	 */
	public function output_markup(): void {
		$json_args = ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ?
			JSON_PRETTY_PRINT :
			0;
		if ( function_exists( 'is_product' ) && is_product() && ! empty( $this->additional_product_markup ) ) {
	 ...
	 * @return string
	 */
	private function schemaize_availability( string $availability ): string {
	 ...
	 * @return array
	 */
	private function generate_aggregate_rating( WC_Product $product ): array {
	 ...
	 * Taken from WC_Structured_Data
	 */
	private function generate_review( WC_Product $product ): array {
	 ...
	 * @return bool
	 */
	private function load_schema_cache( WC_Product $wc_product ): bool {
	 ...
	 * @param WC_Product $wc_product
	 */
	private function save_schema_cache( WC_Product $wc_product ): void {
