...
if ( ! class_exists( 'WCCT_Core' ) ) :
	...
	class WCCT_Core {
		...
		public function __construct() {
			...
			/** Delete transient files, cron added */
			add_filter( 'cron_schedules', array( $this, 'finale_cron_intervals' ), 100 );
			add_action( 'init', array( $this, 'finale_schedule_hook' ) );
			add_action( 'finale_delete_old_transients_cb', array( $this, 'delete_old_transients' ) );
		}

		public function finale_cron_intervals( $schedules ) {
			$schedules['xl_finale_daily_schedule'] = array(
				'interval' => 86400,
				'display'  => 'Once Daily',
			);
		...
		public function finale_schedule_hook() {
			if ( wp_next_scheduled( 'finale_delete_old_transients_cb' ) ) {
				return;
			}
			wp_schedule_event( time() + 10, 'xl_finale_daily_schedule', 'finale_delete_old_transients_cb' );
		}
		...
		public function delete_old_transients() {
			...
			try {
				...
				$file_data = $this->list_files_transient_files();
		...
		public function list_files_transient_files() {
			$uploaded_folder  = wp_upload_dir();
			$xlplugins_folder = $uploaded_folder['basedir'] . '/xlplugins/xl-wp-posts-transient';
			$file_data        = array();
			$three_days_ago   = strtotime( '-3 days' );

			if ( ! is_dir( $xlplugins_folder ) ) {
				return $file_data;
			}

			$iterator = new RecursiveIteratorIterator( new RecursiveDirectoryIterator( $xlplugins_folder ) );
			foreach ( $iterator as $fileInfo ) {
				if ( ! $fileInfo->isFile() ) {
					continue;
				}
				$creation_date = $fileInfo->getCTime();
				if ( $creation_date <= $three_days_ago ) {
		...
		public function hpos_compatibility_declaration() {
			if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
				\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WCCT_PLUGIN_FILE, true );
			}
		}
	}
