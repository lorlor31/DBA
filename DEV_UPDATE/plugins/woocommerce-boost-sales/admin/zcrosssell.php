...
class VI_WBOOSTSALES_Admin_ZCrosssell {
	protected $settings;
    protected static $product_type_of_crossell = array( 'simple', 'variable', 'subscription', 'variable-subscription', 'member', 'lottery' );
				...
				if ( $the_query->have_posts() ) {
					while ( $the_query->have_posts() ) {
						...
						if ( is_array( $woo_c_ids ) && count( $woo_c_ids ) ) {
							...
							$bundle_price_dynamic = $this->settings->get_option( 'bundle_price_dynamic' );
							$level_count          = count( $price_from );
							if ( ! is_array( $bundle_price_dynamic ) || count( $bundle_price_dynamic ) !== $level_count ) {
								$bundle_price_dynamic = array_fill( 0, $level_count, '1' );
							}
							$dynamic_price = array(
								'dynamic_price'   => '',
								'discount_type'   => '',
							...
							if ( is_array( $price_from ) && count( $price_from ) ) {
								...
								if ( $bundle_price_dynamic[ $match ] == 1 ) {
									$dynamic_price['dynamic_price']   = 1;
									$dynamic_price['discount_type']   = $discount_type[ $match ];
									$dynamic_price['discount_amount'] = $discount_value[ $match ];
								} else {
							...
							if ( $dynamic_price['dynamic_price'] ) {
								update_post_meta( $post_id, '_wbs_dynamic_price', $dynamic_price['dynamic_price'] );
								update_post_meta( $post_id, '_wbs_discount_type', $dynamic_price['discount_type'] );
								update_post_meta( $post_id, '_wbs_discount_amount', $dynamic_price['discount_amount'] );
							}
		...
		$arg            = array(
			...
			'tax_query'      => array(
				...
				array(
					'taxonomy' => 'product_type',
					'field'    => 'slug',
					'terms'    => self::$product_type_of_crossell,
					'operator' => 'IN'
				),
			)
		);
	...
	public function wbs_c_save_product() {
		...
		if ( count( $c_id ) ) {
			...
			if ( $new_bundle ) {
				...
				if ( count( $arry_pa ) ) {
					wp_update_post( array( 'ID' => $product_bundle_id, 'post_status' => 'publish' ) );
					...
					if ( $dynamic_price['dynamic_price'] ) {
						update_post_meta( $product_bundle_id, '_wbs_dynamic_price', $dynamic_price['dynamic_price'] );
						update_post_meta( $product_bundle_id, '_wbs_discount_type', $dynamic_price['discount_type'] );
						update_post_meta( $product_bundle_id, '_wbs_discount_amount', $dynamic_price['discount_amount'] );
					} else {
						update_post_meta( $product_bundle_id, '_wbs_dynamic_price', '' );
					}
				}
				$msg['check'] = 'done';
			}
	...
	public function wbs_update_product() {
		...
		$dynamic_price   = filter_input( INPUT_POST, 'dynamic_price', FILTER_SANITIZE_STRING );
		$discount_type   = filter_input( INPUT_POST, 'discount_type', FILTER_SANITIZE_STRING );
		$discount_amount = filter_input( INPUT_POST, 'discount_amount', FILTER_SANITIZE_STRING );
		...
		update_post_meta( $p_id, '_wbs_dynamic_price', $dynamic_price );
		update_post_meta( $p_id, '_wbs_discount_type', $discount_type );
		update_post_meta( $p_id, '_wbs_discount_amount', $discount_amount );
		$msg = array();
		if ( ! is_wp_error( $p_id ) ) {
			$msg['check'] = 'done';
		} else {
	...
	public function enqueue_scripts() {
		...
		if ( $page == 'woocommerce-boost-sales-crosssell' ) {
			...
			wp_enqueue_style( 'woocommerce-boost-sales-crosssell-admin', VI_WBOOSTSALES_CSS . 'crosssell-admin.css', array(), VI_WBOOSTSALES_VERSION );
			...
			wp_localize_script( 'woocommerce-boost-sales-crosssell-admin', 'wbs_crosssell_admin_params', array(
        ...
        <div class="wrap">
			...
			if ( $the_query->have_posts() ) {
                ...
                <form method="get">
                    ...
                    <input type="hidden" name="page" value="woocommerce-boost-sales-crosssell">
                                ...
                                <td data-id="<?php echo $p_id ?>" class="name column-cross-sells"
                                    ...
                                    <p>
                                        <input class="wbs-use-other-bundle"
											...
											echo 'checked' ?>><label
                                                for="<?php echo 'wbs-use-other-bundle-' . $p_id ?>"><?php esc_html_e( 'Use a bundle of another product', 'woocommerce-boost-sales' ) ?></label>
                                    </p>
                                                ...
                                                <div class="inline-edit-row"
                                                    ...
                                                    <fieldset class="">
                                                        ...
                                                        <div class="inline-edit-col">
															...
															?>
                                                            <label>
                                                                <span><?php esc_html_e( 'Use dynamic price', 'woocommerce-boost-sales' ) ?></span>
                                                                <span><input
                                                                            class="text wc_input_price" type="checkbox"
                                                                            name="_wbs_dynamic_price"
                                                                            <?php checked( $dynamic_price, 1 ) ?>
                                                                            value="1"></span>
                                                            </label>
                                                            <label class="wbs-bundle-dynamic-price-fields">
                                                                <span><?php esc_html_e( 'Discount type', 'woocommerce-boost-sales' ) ?></span>
                                                                <span><select name="_wbs_discount_type">
                                                                        <option value="percent" <?php selected( $discount_type, 'percent' ) ?>><?php esc_html_e( 'Percentage(%)', 'woocommerce-boost-sales' ); ?></option>
                                                                        <option value="fixed" <?php selected( $discount_type, 'fixed' ) ?>><?php esc_html_e( 'Fixed amount', 'woocommerce-boost-sales' ); ?></option>
                                                                    </select></span>
                                                            </label>
                                                            <label class="wbs-bundle-dynamic-price-fields">
                                                                <span><?php esc_html_e( 'Discount amount', 'woocommerce-boost-sales' ) ?></span>
                                                                <span><input type="number" min="0" step="any"
                                                                             name="_wbs_discount_amount"
                                                                             value="<?php echo esc_attr( $discount_amount ) ?>"></span>
                                                            </label>
