...
if ( $products_loop->have_posts() ) {
	...
	while ( $products_loop->have_posts() ) : $products_loop->the_post();
		...
		if ( isset( $product ) ) {
			...
			if ( ( ! $product->is_purchasable() || ! function_exists( 'wc_get_product_attachment_props' ) ) && apply_filters('yith_wccp_not_show_not_purchasable_product', true) ) { continue; }
			...
			echo '<li data-product-id="' . $product_id . '" class="ywcp_product_' . $product_id . ' ' . $availability['class'] . '">';
				...
				echo '<div class="ywcp_image_container">';
					...
					echo apply_filters( 'yith_ywcp_thumbnail_image', $image, $post->ID, $product );
				echo '</div>';
				...
				echo '<div class="ywcp_product_info">';
					...
					echo '<div class="ywcp_product_link">';
						if ( apply_filters( 'ywcp_use_quick_view', defined('YITH_WCQV_PREMIUM') ) ) {
							...
							echo '<a href="' . esc_url( $product->get_permalink() ) . '" target="_blank">'
								. apply_filters( 'yith_wcp_product_link_text' , __( 'show the product page' , 'yith-composite-products-for-woocommerce' ) ) . '</a>';
						}
					echo '</div>';
				echo '</div>';
