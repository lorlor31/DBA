...
public function get_addon_display_on_cart($value, $sign, $price, $regular_price, $sale_price) {
    ...
    error_log("ENTERED get_addon_display_on_cart");
    error_log("get_addon_display_on_cart value: $value");
    error_log("get_addon_display_on_cart sign: $sign");
    error_log("get_addon_display_on_cart price: $price");
    error_log("get_addon_display_on_cart regular_price: $regular_price");
    error_log("get_addon_display_on_cart sale_price: $sale_price");

    $display = $value . ( '' !== $price && floatval( 0 ) !== floatval( $price ) ? ' (' . $sign . wc_price( $price ) . ')' : '' );

    // Log the final display value
    error_log("get_addon_display_on_cart display before filter: $display");

    $display = apply_filters('yith_wapo_addon_display_on_cart', $display, $value, $sign, $price, $regular_price, $sale_price);

    error_log("get_addon_display_on_cart display after filter: $display");
    return $display;
}
...
$option_product_qty = 1;
        if (isset($cart_item['yith_wapo_product_qty'][$addon_id . '-' . $option_id])) {
            $option_product_qty = $cart_item['yith_wapo_product_qty'][$addon_id . '-' . $option_id];
        } elseif (isset($cart_item['yith_wapo_qty_options'][$addon_id . '-' . $option_id])) {
            $option_product_qty = $cart_item['yith_wapo_qty_options'][$addon_id . '-' . $option_id];
        }
...
public function get_addon_value_on_cart($addon_id, $option_id, $key, $original_value, $cart_item, $grouped_in_cart = false) {
    error_log("ENTERED get_addon_value_on_cart with addon_id: $addon_id, option_id: $option_id, key: $key, original_value: $original_value");
    ...
    if ('product' === $addon_type) {
        error_log("YITH ADDON original_value " . print_r($original_value, true));
        ...
        error_log("YITH ADDON cart_item yith_wapo_qty_options " . print_r($cart_item['yith_wapo_qty_options'], true));
        error_log("YITH ADDON cart_item yith_wapo_product_qty " . print_r($cart_item['yith_wapo_product_qty'], true));
        error_log("YITH ADDON key " . print_r($key, true));
        error_log("YITH ADDON option_product_qty " . print_r($option_product_qty, true));
        ...
        error_log("YITH ADDON final value: $value");
    ...
    error_log("YITH ADDON get_addon_value_on_cart addon_id: $addon_id, option_id: $option_id, key: $key, original_value: $original_value");
    error_log("YITH ADDON get_addon_value_on_cart cart_item: " . print_r($cart_item, true));
    error_log("YITH ADDON get_addon_value_on_cart value: $value");
