...
class VosfacturesInvoice {
	...
	private function setup_carriage() {
		if ( ( get_option( 'woocommerce_firmlet_settings' )['incl_free_shipment'] == 'yes' ) ||
			...
			foreach ( $this->order->get_shipping_methods() as $shipping_item_obj ) {
				...
				$this->carriage[] = array(
					'name'              => strip_tags( $shipping_item_obj->get_name() ),
					'quantity'          => 1,
					'total_price_gross' => $item_total + $total_tax,
					'tax'               => ( $total_tax == 0 ? 'disabled' : round( (float) $tax, 2 ) ),
					'service'           => true,
				);
			}
		}
	...
	private function add_order_fee() {
		...
		if ( ! empty( $fees ) ) {
			...
			foreach ( $fees as $fee_item_obj ) {
				...
				$this->fees[] = array(
					'name'              => strip_tags( $fee_item_obj->get_name() ),
					'quantity'          => 1,
					'total_price_gross' => $item_total + $total_tax,
					'tax'               => ( $total_tax == 0 ? 'disabled' : round( (float) $tax, 2 ) ),
					'service'           => true,
	 ...
	 */
	private function get_tax_from_line_item( $line_item ) {
	...
	private function setup_positions() {
		...
		foreach ( $line_items as $line_item ) {
			...
			$position_attributes = array(
				...
				'name'              => strip_tags( $line_item->get_name() ),
				'quantity'          => $quantity,
				'total_price_gross' => $total_price_gross,
				'tax'               => ( $usedTax == 0 ? 'disabled' : round( (float) $tax, 2 ) ),
				'discount'          => $discount,
			);
