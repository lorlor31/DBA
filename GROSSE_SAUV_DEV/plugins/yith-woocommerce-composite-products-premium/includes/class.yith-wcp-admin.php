...
if ( ! class_exists( 'YITH_WCP_Admin' ) ) {
    ...
    class YITH_WCP_Admin {
        ...
        public function action_links( $links ) {
            $links = yith_add_action_links( $links, $this->_panel_page, true, YITH_WCP_SLUG );
            return $links;
        }
         ...
         * @use plugin_row_meta
         */
        public function plugin_row_meta( $new_row_meta_args, $plugin_meta, $plugin_file, $plugin_data, $status, $init_file = 'YITH_WCP_INIT' ) {
            if ( defined( $init_file ) && constant( $init_file ) == $plugin_file ) {
                $new_row_meta_args['slug'] = YITH_WCP_SLUG;
            }
...
original shipping weight and dimensions. <strong>Per-Item Shipping</strong> should also be checked if all composite items are virtual.', 'yith-composite-products-for-woocommerce' ),
        ...
        public function composite_write_panel_tabs() {
            ...
            echo '<style>#woocommerce-product-data ul.wc-tabs li.general_tab { display: block !important; } #general_product_data.woocommerce_options_panel .options_group.show_if_simple { display: block !important; }</style>';
        ...
        function admin_html_order_item_class( $class, $item, $order ) {
            if ( is_object( $item ) && ! empty( $item ) ) {
            	...
            	if ( $data && !empty( $data['meta_data'] ) ) {
            		$data_array = $data['meta_data'];
					$class_meta = '';
            		foreach ( $data_array as $meta_item ) {
            			$class_meta .= ' '.$meta_item->get_data()['key'];
					}
				}

            	if( !empty($class_meta) ) {
            		$class = $class . $class_meta;
				}
