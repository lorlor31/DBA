...
if ( ! class_exists( 'YITH_WCP_Frontend' ) ) {
	...
	class YITH_WCP_Frontend {
		 ...
		 *
		 */
		protected $show_component_list = true;
		...
		public function __construct( $wcp_object ) {
            ...
            // Shortcodes.
            add_shortcode( 'yith_wcp_show_component_list', array( $this, 'yith_wcp_show_component_list_shortcode' ) );
            add_action( 'yith_wcp_show_component_list_shortcode', array( $this, 'show_component_list' ) );
		...
		public function show_component_list() {
			global $product;
			if ( is_object( $product ) && $product->get_type() =='yith-composite' && $this->show_component_list ) {
                $this->show_component_list = false;
				wc_get_template( 'ywcp-composite-list.php', array(), '', YITH_WCP_TEMPLATE_FRONTEND_PATH );
			}
		}

        /**
         * Show Component List Shortcode
         */
        public function yith_wcp_show_component_list_shortcode() {
            ob_start();
            global $product;
            if ( is_object( $product ) && $product->get_type() =='yith-composite' && $this->show_component_list ) {
                do_action( 'yith_wcp_show_component_list_shortcode' );
            }
            return ob_get_clean();
        }
		...
		public function ywcp_woocommerce_get_price( $price, $product ) {
            ...
            if(  ! is_archive() &&  ( isset( $_REQUEST['key'] ) || ( is_cart() || is_checkout() ) ) ){
		...
		public static function getAvailabilityHtml( $product ) {
				...
				$availability_html = apply_filters( 'woocommerce_get_stock_html', $availability_html, $product );
