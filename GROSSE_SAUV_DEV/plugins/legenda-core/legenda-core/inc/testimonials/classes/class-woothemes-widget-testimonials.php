 ...
 * - get_orderby_options()
 */
if ( !class_exists('Woothemes_Widget_Testimonials') ) {
	class Woothemes_Widget_Testimonials extends WP_Widget {
		...
		public function __construct() {
			...
			$this->woothemes_widget_description = __( 'Recent testimonials on your site.', 'legenda-core' );
			$this->woothemes_widget_idbase = 'woothemes_testimonials';
			$this->woothemes_widget_title = __( 'Testimonials', 'legenda-core' );
		...
		public function widget( $args, $instance ) {
			...
			$args['before'] = (isset($before_widget)) ? $before_widget : '';
			$args['after'] = (isset($after_widget)) ? $after_widget : '';
			...
			// Integer values.
			if ( isset( $instance['limit'] ) && ( 0 < $instance['limit'] ) ) { $args['limit'] = intval( $instance['limit'] ); }
			if ( isset( $instance['specific_id'] ) && ( 0 < $instance['specific_id'] ) ) { $args['id'] = intval( $instance['specific_id'] ); }
			if ( isset( $instance['size'] ) && ( 0 < $instance['size'] ) ) { $args['size'] = intval( $instance['size'] ); }
	...
	?>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title (optional):', 'legenda-core' ); ?></label>
				<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>"  value="<?php echo $instance['title']; ?>" class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" />
			</p>
			<!-- Widget Limit: Text Input -->
			<p>
				<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Limit:', 'legenda-core' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php _e( 'Image Size (in pixels):', 'legenda-core' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Order By:', 'legenda-core' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e( 'Order Direction:', 'legenda-core' ); ?></label>
			...
			<!-- Widget Display Navigation: Checkbox Input -->
	       	<p>
	        	...
	        	<label for="<?php echo $this->get_field_id( 'navigation' ); ?>"><?php _e( 'Display Control Navigation', 'legenda-core' ); ?></label>
		   	</p>
			<!-- Widget Display Author: Checkbox Input -->
	       	<p>
	        	...
	        	<label for="<?php echo $this->get_field_id( 'display_author' ); ?>"><?php _e( 'Display Author', 'legenda-core' ); ?></label>
		   	</p>
			<!-- Widget Display Avatar: Checkbox Input -->
	       	<p>
	        	...
	        	<label for="<?php echo $this->get_field_id( 'display_avatar' ); ?>"><?php _e( 'Display Avatar', 'legenda-core' ); ?></label>
		   	</p>
	       	...
	       	<p>
	        	...
	        	<label for="<?php echo $this->get_field_id( 'display_url' ); ?>"><?php _e( 'Display URL', 'legenda-core' ); ?></label>
		   	</p>
		   	<!-- Widget Category: Select Input -->
			<p>
				<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Category:', 'legenda-core' ); ?></label>
				<?php
					$dropdown_args = array( 'taxonomy' => 'testimonial-category', 'class' => 'widefat', 'show_option_all' => __( 'All', 'legenda-core' ), 'id' => $this->get_field_id( 'category' ), 'name' => $this->get_field_name( 'category' ), 'selected' => $instance['category'] );
					wp_dropdown_categories( $dropdown_args );
				?>
			</p>
			<!-- Widget ID: Text Input -->
			<p>
				<label for="<?php echo $this->get_field_id( 'specific_id' ); ?>"><?php _e( 'Specific ID (optional):', 'legenda-core' ); ?></label>
			...
			<p><small><?php _e( 'Display a specific testimonial, rather than a list.', 'legenda-core' ); ?></small></p>
	<?php
		} // End form()
		...
		protected function get_orderby_options () {
			return array(
						'none' => __( 'No Order', 'legenda-core' ),
						'ID' => __( 'Entry ID', 'legenda-core' ),
						'title' => __( 'Title', 'legenda-core' ),
						'date' => __( 'Date Added', 'legenda-core' ),
						'menu_order' => __( 'Specified Order Setting', 'legenda-core' ),
						'rand' => __( 'Random Order', 'legenda-core' )
						);
		...
		protected function get_order_options () {
			return array(
						'ASC' => __( 'Ascending', 'legenda-core' ),
						'DESC' => __( 'Descending', 'legenda-core' )
						);
