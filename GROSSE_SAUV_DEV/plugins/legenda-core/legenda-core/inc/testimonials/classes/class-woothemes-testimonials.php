...
if ( !class_exists('Woothemes_Testimonials') ) {
	class Woothemes_Testimonials {
		...
		public function register_post_type () {
			$labels = array(
				'name' => _x( 'Testimonials', 'post type general name', 'legenda-core' ),
				'singular_name' => _x( 'Testimonial', 'post type singular name', 'legenda-core' ),
				'add_new' => _x( 'Add New', 'testimonial', 'legenda-core' ),
				'add_new_item' => sprintf( __( 'Add New %s', 'legenda-core' ), __( 'Testimonial', 'legenda-core' ) ),
				'edit_item' => sprintf( __( 'Edit %s', 'legenda-core' ), __( 'Testimonial', 'legenda-core' ) ),
				'new_item' => sprintf( __( 'New %s', 'legenda-core' ), __( 'Testimonial', 'legenda-core' ) ),
				'all_items' => sprintf( __( 'All %s', 'legenda-core' ), __( 'Testimonials', 'legenda-core' ) ),
				'view_item' => sprintf( __( 'View %s', 'legenda-core' ), __( 'Testimonial', 'legenda-core' ) ),
				'search_items' => sprintf( __( 'Search %s', 'legenda-core' ), __( 'Testimonials', 'legenda-core' ) ),
				'not_found' =>  sprintf( __( 'No %s Found', 'legenda-core' ), __( 'Testimonials', 'legenda-core' ) ),
				'not_found_in_trash' => sprintf( __( 'No %s Found In Trash', 'legenda-core' ), __( 'Testimonials', 'legenda-core' ) ),
				'parent_item_colon' => '',
				'menu_name' => __( 'Testimonials', 'legenda-core' )
		...
		public function register_custom_column_headings ( $defaults ) {
			$new_columns = array( 'image' => __( 'Image', 'legenda-core' ) );
		...
		public function updated_messages ( $messages ) {
		  ...
		  $messages[$this->token] = array(
		    ...
		    1 => sprintf( __( 'Testimonial updated. %sView testimonial%s', 'legenda-core' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
		    2 => __( 'Custom field updated.', 'legenda-core' ),
		    3 => __( 'Custom field deleted.', 'legenda-core' ),
		    4 => __( 'Testimonial updated.', 'legenda-core' ),
		    /* translators: %s: date and time of the revision */
		    5 => isset($_GET['revision']) ? sprintf( __( 'Testimonial restored to revision from %s', 'legenda-core' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		    6 => sprintf( __( 'Testimonial published. %sView testimonial%s', 'legenda-core' ), '<a href="' . esc_url( get_permalink( $post_ID ) ) . '">', '</a>' ),
		    7 => __('Testimonial saved.', 'legenda-core'),
		    8 => sprintf( __( 'Testimonial submitted. %sPreview testimonial%s', 'legenda-core' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
		    9 => sprintf( __( 'Testimonial scheduled for: %1$s. %2$sPreview testimonial%3$s', 'legenda-core' ),
		      // translators: Publish box date format, see http://php.net/date
		      '<strong>' . date_i18n( __( 'M j, Y @ G:i', 'legenda-core' ), strtotime( $post->post_date ) ) . '</strong>', '<a target="_blank" href="' . esc_url( get_permalink($post_ID) ) . '">', '</a>' ),
		    10 => sprintf( __( 'Testimonial draft updated. %sPreview testimonial%s', 'legenda-core' ), '<a target="_blank" href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) . '">', '</a>' ),
		  );
		...
		public function meta_box_setup () {
			add_meta_box( 'testimonial-data', __( 'Testimonial Details', 'legenda-core' ), array( &$this, 'meta_box_content' ), $this->token, 'normal', 'high' );
		} // End meta_box_setup()
		...
		public function enter_title_here ( $title ) {
			if ( get_post_type() == $this->token ) {
				$title = __( 'Enter the customer\'s name here', 'legenda-core' );
			}
			return $title;
		...
		public function get_custom_fields_settings () {
			...
			$fields['gravatar_email'] = array(
			    'name' => __( 'Gravatar E-mail Address', 'legenda-core' ),
			    'description' => sprintf( __( 'Enter in an e-mail address, to use a %sGravatar%s, instead of using the "Featured Image".', 'legenda-core' ), '<a href="' . esc_url( 'http://gravatar.com/' ) . '" target="_blank">', '</a>' ),
			    'type' => 'text',
			    'default' => '',
			    'section' => 'info'
			);
			...
			$fields['byline'] = array(
			    'name' => __( 'Byline', 'legenda-core' ),
			    'description' => __( 'Enter a byline for the customer giving this testimonial (for example: "CEO of WooThemes").', 'legenda-core' ),
			    'type' => 'text',
			    'default' => '',
			    'section' => 'info'
			);
			...
			$fields['url'] = array(
			    'name' => __( 'URL', 'legenda-core' ),
			    'description' => __( 'Enter a URL that applies to this customer (for example: http://woothemes.com/).', 'legenda-core'  ),
			    'type' => 'url',
			    'default' => '',
			    'section' => 'info'
			);
