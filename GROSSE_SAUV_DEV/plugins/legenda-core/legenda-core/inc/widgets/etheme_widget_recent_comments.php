<?php 
	class Etheme_Recent_Comments_Widget extends WP_Widget {
	    ...
	    function __construct() {
	        $widget_ops = array('classname' => 'etheme_widget_recent_comments', 'description' => __( 'The most recent comments (Etheme edit)', 'legenda-core' ) );
	        parent::__construct('etheme-recent-comments', '8theme - '.__('Recent Comments', 'legenda-core'), $widget_ops);
	    ...
	    function widget( $args, $instance ) {
		    if (etheme_admin_widget_preview(esc_html__('8theme - Recent Comments', 'woopress-core')) !== false) return;
	        ...
	        if ( $comments ) {
	            foreach ( (array) $comments as $comment) {
	                ...
	                $output .=  '<li class="recentcomments">';
	                    ...
	                    $output .=  ' '.__('by', 'legenda-core').' <span class="comment_author">'.get_comment_author_link().'</span>';
	                $output .=  '</li>';
	            }
	        }
	        $output .= '</ul>';
	...
	?>
	        ...
	        <p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of comments to show:', 'legenda-core'); ?></label>
