		...
		if ( true === $display_on_checkout || ! is_checkout() ) {
			...
			if ( 'single' === $type && is_singular( 'product' ) ) {
				if ( isset( $data['single_bar'] ) && is_array( $data['single_bar'] ) && count( $data['single_bar'] ) > 0 ) {
					...
					if ( ! empty( $goals ) && isset( $goals['type'] ) && 'same' === $goals['type'] && ! $manage_stock_check ) {
						$show_bar = false;
					}
                    
					if ( $show_bar && ! empty( $goals ) && isset( $goals['type'] ) && 'same' === $goals['type'] && in_array( $product->get_type(), WCCT_Common::get_variable_league_product_types(), true ) && WCCT_Common::get_total_stock( $product ) <= 0 ) {
			...
			if ( 'grid' === $type ) {
				...
				if ( isset( $data['grid_bar'] ) && is_array( $data['grid_bar'] ) && count( $data['grid_bar'] ) > 0 ) {
					...
					if ( ! empty( $goals ) && 'same' === $goals['type'] && $show_bar && in_array( $product->get_type(), WCCT_Common::get_variable_league_product_types(), true ) && WCCT_Common::get_total_stock( $product ) <= 0 ) {
