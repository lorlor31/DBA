    ...
    <form class="woocommerce-boost-sales-cart-form" method="post" enctype='multipart/form-data'>
		...
		if ( $bundled_items ) {
			...
			foreach ( $bundled_items as $bundled_item ) {
				...
				if ( $bundled_product->get_type() == 'variation' ) {
					$parent_id = $bundled_product->get_parent_id();
					$parent    = wc_get_product( $parent_id );
					$title     = $parent->get_title();
					$permalink = $parent->get_permalink();
				} else {
					$title     = $bundled_product->get_title();
					$permalink = $bundled_product->get_permalink();
				}
				?>
                <tr>
                    <td class="wbs-wcpb-product-bundled-item-image">
						<?php echo $bundled_product->get_image(); ?>
                    </td>
                    <td class="wbs-wcpb-product-bundled-item-data">
                        <div>
                            <a href="<?php echo esc_url( $permalink ); ?>">
								<?php echo absint( $quantity ) . ' x ' . wp_kses_post( $title ); ?>
                            </a>
                        </div>
                        <p>
							<?php
							echo $bundled_product->get_type() == 'variation' ? wp_kses_post( $parent->get_short_description() ) : wp_kses_post( $bundled_post->post_excerpt );
							?>
                        </p>
						<?php
        ...
        <button type="submit" class="single_add_to_cart_button button alt"><?php echo esc_html__( 'Add to cart', 'woocommerce' ); ?></button>
