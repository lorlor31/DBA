	...
	public function add_order_item_cost( $product, $item, $item_id ) {
		...
		if ( ! $item || ! $product instanceof \WC_Product ) {
			...
			<td class="item_cost_of_goods" width="1%">
				...
				if ($product_id == 7332 || $product_id == 7310 || $product_id == 7321 || $product_id == 7298 || $parent_id == 7298 || $parent_id == 7332 || $parent_id == 7310 || $parent_id == 7321 ){
					echo '<span style="color:red;">doit être vendu par un multiple de 3</span> ' ;
				}
				
				if ( $classe_expedition == 'bureau' && $supplier == "Vinco" ) {
					echo '<span style="color:red;position:absolute;">/!\ Si Drop</span> ' ;
				}	
				?>	
				</div>		
