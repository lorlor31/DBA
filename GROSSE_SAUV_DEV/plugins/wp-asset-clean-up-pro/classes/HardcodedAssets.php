...
class HardcodedAssets
{
     ...
     *
     * Possible params:
     * "encode_it" - if set to "false", it's mostly for testing purposes
     * "is_guest_visit" - if set to "false", it's mostly for testing purposes)
	 *
	 * @return string|array
	 */
	public static function getAll($htmlSource, $params = array('encode_it' => true, 'is_guest_visit' => false))
	{
		...
		// Then do not fetch all the hardcoded assets, but only the ones for which at least one marked for unloading (e.g., for inline SCRIPTs) exist
		if (isset($params['is_guest_visit']) && $params['is_guest_visit']) {
			$hardCodedAssetsMarkedForUnload = HardcodedAssetsPro::getHardcodedUnloadList();
			 ...
			 * wpacu_hardcoded_scripts_noscripts_inline
			 */
			...
			if (! isset($hardCodedAssetsMarkedForUnload['wpacu_hardcoded_scripts_src']) && ! isset($hardCodedAssetsMarkedForUnload['wpacu_hardcoded_scripts_noscripts_inline'])) {
		...
		if (isset($params['encode_it']) && $params['encode_it']) {
