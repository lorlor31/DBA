...
class YITH_YWRAQ_Order_Request {
	...
	public function checkout_fields_manage() {
		...
		if ( 'both' === $this->order_payment_info['checkout_info'] || 'billing' === $this->order_payment_info['checkout_info'] ) {
			foreach ( WC()->countries->get_address_fields( '', 'billing_' ) as $key => $value ) {
				$this->order_payment_info[ $key ] = $order->get_meta( '_' . $key);
	...
	public function save_quote( $order ) {
		if ( $this->saved_quote || ! isset( $_POST['yit_metaboxes'], $_POST['post_ID'] ) || ! isset( $_POST['yit_metaboxes']['_ywraq_safe_submit_field'] ) || empty( $_POST['yit_metaboxes']['ywraq_customer_email'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}
		...
		if ( apply_filters( 'ywraq_override_shipping_method', ywraq_is_true( $override_shipping ) ) ) {
			...
			if ( $shipping_items ) {
				...
				foreach ( $rates as $key => $rate ) {
					...
					foreach ( $shipping_items as $shipping_item ) {
						 if ( $shipping_item !== null && method_exists($shipping_item, 'get_method_id') ) { // BYME
							...
							if ( $shipping_item_articles_final === $rate_articles_without_quantity ) {
								...
								if ( ! empty( $tt['total'] ) ) {
									foreach ( $tt['total'] as $k => $t ) {
										$tax_value_as_float = floatval($t);
										if ( isset( $taxes_value[ $k ] ) ) {
											$taxes_value[ $k ] = $taxes_value[ $k ] + $tax_value_as_float;
										} else {
											$taxes_value[ $k ] = $tax_value_as_float;
										}
									}
								}
