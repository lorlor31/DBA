						...
						if ( ! empty( $raq['yith_wapo_individual_item'] ) && 1 === $raq['yith_wapo_individual_item'] && ! empty( $raq['yith_wapo_parent_key'] ) ) :
							...
							<tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>"
								...
								<td class="product-name"
									...
									if ( ! empty( $raq['yith_wapo_options'] ) ) {
    echo '<br><ul style="margin-left: 10px;">';
    foreach ( $raq['yith_wapo_options'] as $addon_key => $addon ) {
        if ( isset( $addon['quantity'] ) && $addon['quantity'] > 0 ) {
            $addon_name = isset( $addon['name'] ) ? $addon['name'] : 'Addon';
            echo '<li><strong>' . esc_html( $addon_name ) . '</strong>: ' . esc_html( $addon['quantity'] ) . '</li>';
        }
    }
    echo '</ul>';
}
