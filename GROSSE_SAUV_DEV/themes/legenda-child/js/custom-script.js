...
jQuery( function( $ ) {
    ...
    // Rajouter une coche à l'élément de l'option s'il est cliqué
    $(document).on('click', '.swatch-anchor', function(event) {
        var elementClique = $(this);
        var parent = elementClique.parent();
        var grandparent = elementClique.parent().parent() ;
        // Si l'élément'n'est pas cochée
        if (!parent.find('#check-icone').length) {
            // Si un autre élément est déjà coché, le décocher
            if (grandparent.find('#check-icone').length) {
                var checkIcone=grandparent.find('#check-icone');
                checkIcone.remove();
            }
            // Cocher l'élément
            var icone = $('<img src="https://www.armoireplus.fr/wp-content/plugins/yith-woocommerce-advanced-product-options-premium/assets/img/icon-check.png" id="check-icone">');
            var swatchAnchors = grandparent.find('a.swatch-anchor');
            swatchAnchors.removeAttr('id', 'color-selector');
            elementClique.attr('id', 'color-selector');
            elementClique.prop("selected", true);
            // Régler l'overflow du conteneur pour voir la coche par-dessus
            $('.swatch-control').css('overflow', 'visible');
            // Afficher le descriptif de l'option à la ligne
            $('table.variations .swatch-label ').css('display', 'block');
            parent.prepend(icone) ;
        }
        // Si l'élément' est déjà coché
        else {
            elementClique.removeAttr('id', 'color-selector');
            elementClique.removeClass('selected');
            elementClique.prop("selected", false);
            var checkIcone=parent.find('#check-icone');
            checkIcone.remove();
        }
    });
